...
## ⚖️ 라이선스 — 무엇을 남이 가져다 써도 되나 (2026-08-06 확정)

**저작권자는 선생님 본인**이고, 조건도 본인이 정합니다. 다만 **저장소 전체에 하나의 라이선스를 걸 수 없어** 층으로 나눴습니다.

| 층 | 조건 |
|---|---|
| 대본 · 문법 설명 · 어휘 · 퀴즈 · 문서 · 코드 | **CC BY-NC-SA 4.0** — 출처 밝히고 · 비영리로만 · 같은 조건으로 공유 |
| **TTS 음성 mp3 · 슬라이드 이미지 · 영상** | **제외.** 사이트 내 재생만 허용, 다운로드·재배포 금지 |
