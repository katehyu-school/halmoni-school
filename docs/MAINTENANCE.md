...
| 8 | **TTS·슬라이드 제작 도구 계정** | ElevenLabs, Google Vids 등 (선생님 개인 계정) |
...
> **텍스트만 고치고 재생 없이 바로 다운로드하면 오디오가 갱신되지 않습니다.**
...
> ⚠️ 색인(`renderIdxBody`)은 `ep.vocab`만 인덱싱합니다. **Real Life 지문의 단어는 vocab 카드로도 등록해야 색인·복습에 잡힙니다.**
...
| **L6 마감 테스트 듣기 5개 미녹음** | `data/nhs/L6/TTS/closing/` **비어 있음**(다른 레벨은 5~8개 존재). 파일을 넣으면 자동 연결, 그전까지는 브라우저 TTS 폴백 |
