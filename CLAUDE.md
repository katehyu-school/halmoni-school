...
| 앱 | 현재 상태 | 다음 작업 |
|----|---------|---------|
| **HQ (nhs.html)** | L1~L5 전편+마감테스트 완성 ✅ / **색인 전용 어휘풀(광맥 225개) ✅** / **복습 플래시카드 누적+가중 ✅** / **빠른복습 L2~L6 자동생성 ✅** / **🆕 L6 신설·부팅 완료 ✅** (조건부 탭·영상모드·접이식 자막) — **L6 ep01~03 완성** (경제기사·재개발뉴스·노사토론) | **L6 ep04~12 순차 제작** (ep05 슬라이드/소설 3편 교체작 mp4 대기) |
...
> ✅ **2026-07-29 완료 (L6 신설 세션)** — 다음 세션은 이 블록부터 볼 것:
> — **색인 전용 어휘 풀 신설**: `data/nhs/glossary_pool.json` + nhs.html `renderIdxBody`에 병합(색인엔 `실생활` 배지로 뜨지만 `<새어휘>` 카드는 아님 — `buildVocab` 무수정). 광맥 후보(`HQ_RealLife_광맥_후보_2026-07-26.xlsx`) 649행을 형태소분석기(kiwipiepy)로 활용형→원형 정규화해 **내용어 225개** 정제(보미·않다·이러만 제외, 선생님 지시) 후 편별 등록. 정제 시트 `HQ_광맥_색인후보_정제_2026-07-27.xlsx`.
> — **음절 편 시각자료 이식**: `sejong-korean_v1.html` 한글기초 음절편(천지인 원리·모음조합 pill·레고블록 시각화)을 nhs `Start Here›한글을 배워요›음절` 빌더 **앞**에 삽입(`SYL_VISUAL` 백틱 템플릿). 필요 CSS 6종(`.syl-block`·`.vowel-origin-pill`·`.h-section-title` 등) `core/nhs.css`에 추가. nhs 기존 빌더는 유지.
> — **복습 플래시카드 대수정**: ① 레벨 매핑이 **L1/L2만** 인식하던 버그→L1~L6 인식. ② **누적**(현재 레벨까지 하위 전부 포함) + **하위 비중 축소 가중 샘플링**(`_fcWeightedPick`, 가중치=2^(레벨−현재); 예 L5세션 L5≈52%/L1≈3%). ③ 편 칩→**레벨 칩**(전체/Lv1..). ④ 복습표시 먼저+나머지 랜덤(선생님 선택). `_fcCardId`에 level 포함(레벨간 epId 충돌 방지), cap 6. ⚠️ 복습표시는 로그인 프로필 있을 때만 저장됨.
> — **빠른 복습(에피소드 열 때 워밍업) 수정**: 정적 `WARMUP_POOL`(L1 전용 36문항)을 **L2~L6는 `_buildVocabWarmup`으로 이전 편 어휘 '뜻 맞추기' 자동생성**으로 교체(L1은 기존 유지). "L5에서 L1 기초만 반복" 문제 해결. 참고: '빠른 복습'(`_wu*`)은 플래시카드(`_fc*`)와 별개 서브시스템.
> — **🆕 L6 신설 (어드밴스드: 뉴스·논술·문학·드라마)**: nhs.html에 **Level 6 부팅**(상단 탭·`L6_EPISODES`·`buildSidebar` L6분기·`setLevel` 매핑·색인 prefetch·플래시/워밍업 cap6·trial잠금). **조건부 탭 도입**(`_tabDefs`로 콘텐츠 있는 탭만 표시; `fillAllPanels`을 `fill(id,fn)` 가드로 바꿔 빈 탭 빌더 미호출) — L1~L5에도 안전. **영상 모드 자막=접이식**(`buildVideoScript`의 대본을 `<details>`로). 🔧 **showTab 버그 수정**: 고정 8탭 인덱스로 버튼 활성화하던 걸 **onclick 이름 매칭**으로(조건부 탭에서 Real Life 안 열리던 문제 해결).
>   - **영상 에피소드 스키마**: `video`(mp4 경로) + `characters`(id/name/emoji/color) + `script`(speaker/text/en 자막). 슬라이드 없음. mp4는 standby `data/nhs/L6/slides/epNN/*.mp4` → 프로젝트 `data/nhs/L6/videos/epNN.mp4`로 복사(총 12편 ~70MB, GitHub Pages 호스팅 OK).
>   - **완성**: ep01 물가상승 경제기사(쓰기 실용문·격식체 종결) / ep02 인천 재개발 뉴스(듣기·-다고 강조하다) / ep03 삼총사 노사 포럼(토론·공적 대화 틀·**권리** 포함). **편마다 이해문제·Usage·Quiz·문법카드를 스크립트에 없어도 자동 보완**(선생님 승인). real_life 필드명 주의: tip=`content_ko`, reading=`passage`+`questions`, writing_guided=`prompt_ko`+`model`.
>   - **⚖️ 저작권 교체 확정(PDF 번호 유지)**: 사랑방손님과어머니(주요섭 d.1972)→**메밀꽃 필 무렵(이효석)** / 소나기(황순원 d.2000)→**운수 좋은 날(현진건)** / 자전거도둑(박완서 d.2011)→**감자(김동인)**. **동백꽃(김유정) 유지**. 셋 다 PD(사후 50년 경과). 구절 정리 `L6_소설_교체_인용구절_PD.md`. ⚠️ **교체작 슬라이드·mp4는 선생님이 새로 제작**(기존 저작권작 mp4는 사용 불가).
>   - **L6 전체 구조(원본 `HQ Level 6 script.pdf`, uploads)**: ep04 논술문(비용과 배움의 가치, 신제품·사용료·수업료·공통점·차이점) / ep05 논설문 작성요령(**Slides, mp4 아님** — AI번역기 주제, L5 ep01 재활용) / ep06 메밀꽃 / ep07 동백꽃 / ep08 운수좋은날 / ep09 감자 / ep10 저녁노을(수필, 선생님 오리지널·Kids 출처) / ep11 산책하러 갈까(드라마, 오리지널·Kids) / ep12 필수 감정어휘 30개.
>   - **🔜 다음 세션**: **ep04부터** 순차 제작. **ep12 감정어휘 30개는 Claude가 채우기로**(PDF엔 ~20개만). 추가 드라마 mp4(계란은안돼·영원한사랑)는 **보류**. 소설 3편은 교체작 슬라이드/mp4 준비되면.
> — **⚠️ 방침 변경**: 이전 "**L5가 커리큘럼의 끝 — L6 아님**"([[hq-topik-news-coverage]] 등) 방침은 **폐기** — 선생님이 L6를 정식 레벨로 신설. 미커버 어휘도 L6 뉴스/논술 편에서 커버됨(부록 아님).
...
1. **HQ 쓰기 기능 확장** — 파일럿 현황: 받아쓰기(L3+ 전편 자동)·문장완성(L5 전편, `writing_pool.json`)·가이드 작문(Real Life `writing_guided`, **ep12에만**). **다음 세션 이어서 할 일**: ① 문장완성을 **L4→L3로 확장**(각 편 핵심문법 2문항, writing_pool에 `L4_epNN`/`L3_epNN` 추가), ② **가이드 작문 카드를 다른 편에 확산**(모범답안은 각 편 논술 Real Life 재활용 가능), ③ **L5 문장완성 문항 선생님 QA**(자동작성이라 어색한 답안 점검·보완).
2. **L5 완결 마무리 확인** — ep01~12 + 마감테스트 + 대단원 완성. 마감테스트 듣기 녹음 연결됨. (L5는 콘텐츠상 완료)
