...
| 앱 | 현재 상태 | 다음 작업 |
|----|---------|---------|
| **HQ (nhs.html)** | L1~L5 전편+마감테스트 완성 ✅ / **색인 어휘풀 225개 ✅** / **복습 플래시카드 누적+가중 ✅** / **빠른복습 자동생성 ✅** / **🆕 L6 ep01~09·ep12 완성 ✅** (뉴스·논술·문학·감정어휘 10편) | **L6 ep10 수필·ep11 드라마** (선생님 창작+영상 대기) → 그다음 **전편 리뷰·가다듬기** |
...
> — **미커버 TOPIK 필수어 배치**: 선생님이 체크리스트 F열로 "이 단어→이 편" 매핑한 20개를 기존 편에 배치(고등학생/대학원생/연구원→L1 ep02, 출퇴근·승진→L3 ep05, 요금제→L4 ep10, 자연재해→L5 ep06, 소비자→L4 ep07, 신입생→L5 ep01 등). **L5 ep07에 TOPIK식 뉴스 읽기 지문 + `📺 뉴스 어휘` 세트 신설.** ⚠️ 색인(`renderIdxBody`)은 `ep.vocab`만 인덱싱 → **Real Life 지문 단어는 반드시 vocab 카드로도 등록해야 색인·복습됨**. 상세 [[hq-topik-news-coverage]].
...
1. **L6 마감테스트 듣기 5개 녹음 연결** — TTS 크레딧 복구(8/13) 후 `data/nhs/L6/TTS/closing/ct_listen_01~05.mp3`만 넣으면 자동 연결. 그전까지는 브라우저 TTS 폴백으로 동작.
2. **오답 → 해당 편으로 이동 링크** — 마감테스트 결과 화면이 지금은 '다시 풀기' 한 줄뿐. 틀린 문항의 `ep`를 눌러 그 에피소드로 바로 가게 하면 총괄평가가 학습 루프가 됨.
3. **L6 스키마 결손 보완** — L1~L5는 12/12편에 발음·반말존댓말 섹션이 있는데 L6는 0/12. L6에는 **낭독 발음**(소설 고어체·방언, 뉴스 앵커 문체의 연음)과 **문체·화계 분석**(드라마·소설의 반말↔존댓말 전환 = 심정 추론 도구) 형태로 신설.
4. **L6 ep12 감정어휘 30개 재분류** — 한 편 신규 30개는 인지부하 초과(권장 상한 15). '참고 자료' 성격으로 규정하고 실제 학습은 ep06~11에 6개씩 분산.
...
6. **모바일 앱(hq-mobile.html) 웹 진도 반영** — 주 1회 수업 사이 6일의 공백을 메우는 게 앱의 존재 이유인데 정작 L4~L6 콘텐츠가 연동 안 됨. 오랫동안 미업데이트.
