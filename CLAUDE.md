...
> — **미해결(다음에 필요하면)**: slug가 바뀐 L1·L2 8개 URL 중 2개는 새 slug로 명확히 매칭됨(`l1-ep07-inability-more-ep08.html`→`l1-ep07-inability-more-l2.html`, `l1-ep09-counter-words-which-one-for-what.html`→`l1-ep09-counter-words-which.html`), 나머지 6개는 제목 자체가 안 맞아서 매칭 불가(콘텐츠가 그 사이 실제로 바뀐 것으로 추정): `l1-ep05-connecting-two-actions-order.html`, `l1-ep12-coming-level-2-verb-modifiers.html`, `l2-ep03-because-reason.html`, `l2-ep07-doing-something-someone-respectful.html`, `l2-ep07-person.html`, `l2-ep11-realization-from-what-you-were-told-oh-i.html`. 예전 `hq-mobile→dr-mobile`처럼 301 리다이렉트 스텁을 만들어줄지는 아직 결정 안 함 — 사이트가 아직 2주 정도밖에 안 됐고 검색엔진에 얼마나 인덱싱됐을지 불확실해서 SEO 영향은 낮을 것으로 판단되지만, 선생님이 원하면 다음에 만들 수 있음.
