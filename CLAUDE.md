...
| 앱 | 현재 상태 | 다음 작업 |
|----|---------|---------|
| **HQ (nhs.html)** | L1 ep01~12 완성 ✅ / L2 ep01~12 완성 ✅ / **L1·L2 문법 annotation 툴팁 완성 ✅** / **L3 ep01~12 완성 ✅** / **L3 마감 테스트 완성 ✅** | - |
| **HQ Kids (korean-app_v2.html)** | L1 완성 ✅ / L2 완성 ✅ / L3 unit01~10 완성 ✅ / **L4 unit01~04 완성 ✅** | L4 unit05+ 슬라이드/TTS 준비 후 구현 |
...
### 🐱 다음 세션 — HQ Kids 모바일 앱 인수인계 (2026-06-15)

**현재 상태 (hq-mobile-prototype.html)**
- `scr-kids-map`: 모험 맵 5노드 — 🔤모음1, 🔠자음1 (완료 표시, 클릭해도 토스트만), 🧱블록 조합 (유일하게 실제 동작 — `startGame()`), 🧩단어 만들기/🏗️받침 쌓기 (잠김, 토스트만)
- `scr-kids-game`: 블록 조합 게임 — 자음+모음 2슬롯 탭 방식, 3라운드, Web Speech TTS (`gListen()`)
- 하단 네비 🎮게임/⭐내 별 탭 — 둘 다 미구현 (토스트만)
- HQ(틴/성인) 트랙은 RP_DATA/KK_DATA ep01~12 전부 완성 — Kids는 그 수준에 비해 매우 미니멀

**참고할 기존 설계 (CLAUDE.md "HQ Kids — Level 1 전면 개편" 섹션)**
- 레고블럭 색상 고정: 자음=빨강 / 모음=초록 / 받침=파랑
- 탭 방식 (드래그 ❌), 큰 UI (아이 손가락 친화)
- 받침 단계는 3층 구조 (자음 상 / 모음 중 / 받침 하)
- 정답 피드백: 🎉 + TTS 자동재생, 오답은 블록 튕김

**콘텐츠 소스**
- `korean-app_v2.html`의 Level 1 JSON (`data/elem/level1/unit0N.json`)이 이미 완성되어 있음 — 모바일 마이크로러닝 콘텐츠는 이걸 재가공해서 가져오는 방식이 자연스러움 (전면 재설계 ❌, [[fix-over-rebuild-preference]])

**제안 작업 순서**
1. 모험 맵 잠긴 노드(단어 만들기/받침 쌓기) 중 하나를 블록 조합과 같은 패턴으로 확장
2. ⭐내 별 탭 — 별 카운터(`#kStars`)를 실제 진행 상황과 연동 (현재 하드코딩 12)
3. 위 작업 안정화 후 PWA 매니페스트 등 공통 기반 작업 진행
...
**등급 기준** (점수 화면에 표시):
- 🏆 **A** (90%+): TOPIK 3급 도전 준비 완료!
- 🌟 **B** (80%+): Level 3 충분히 마스터!
- 👍 **C** (70%+): 조금 더 복습 후 도전!
- 🌱 **D** (70% 미만): 다시 한번 복습해요

**구현 함수**: `loadL3Test()`, `renderL3TPage()`, `renderL3T()`, `l3tChoose(idx)`, `l3tNext()`, `renderL3TScore()`, `l3tRetry()`  
**대상 레벨**: TOPIK 3급 / CEFR B1 입구 수준  
**문항 구성**: mc 10(ep02,05,06,07,08,10,11,10,11,12 문법) + 읽기 4(지문 2개) + 듣기 5(ep01,03,09,11,12)
