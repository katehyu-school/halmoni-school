...
| 앱 | 현재 상태 | 다음 작업 |
|----|---------|---------|
| **HQ (nhs.html)** | L1 ep01~12 완성 ✅ / L2 ep01~12 완성 ✅ / **L1·L2 문법 annotation 툴팁 완성 ✅** / **L3 ep01~12 완성 ✅** / **L3 마감 테스트 완성 ✅** / **L4 ep01~12 완성 ✅** / **L4 마감 테스트 완성 ✅** / **L5 신규 오픈 — ep01 완성 ✅** | L5 ep02+ 스크립트 준비되면 진행 |
...
| `data/nhs/L5/ep0N.json` | Hangeul Quest Level 5 에피소드 (ep01 완성 — "삼총사의 한국 정착기" 시리즈 시작) |
...
> ✅ **슬라이드 vs TTS 오디오 불일치 — 해결 완료 (2026-07-13)**: 19번째 슬라이드(스크립트 18번째 줄, "우리는 ___에서 같이 한국어를 공부했어")에서 처음엔 슬라이드="한글 퀘스트에서" vs TTS 파일명="캐나다에서"로 불일치가 있어 제가 "TTS가 최신 수정"이라고 잘못 추측하고 텍스트를 "캐나다에서"로 넣었었음. **선생님 설명으로 원인 파악**: (1) TTS 툴에서 지문만 고치고 플레이 없이 바로 다운로드하면 반영이 안 되는 버그가 있어서 오디오가 옛날 내용 그대로였을 뿐, "캐나다에서"는 실수였음. (2) 브랜드명 노출 회피 원칙은 **다른 상업 브랜드**를 슬라이드에서 HQ로 바꾸는 용도이고, 대사에는 원래 적용 안 됨. (3) 미라(캘리포니아)·올리비아(캐나다)는 출신 국가가 달라서 "같이 공부한 곳"의 접점을 만들려고 **의도적으로 "한글 퀘스트"(앱 자체)를 스토리 안에 넣은 것** — 실수가 아니라 설정. 선생님이 오디오를 재생 후 재다운로드해서 새 파일(`audio_17_우리는_한글_퀘스트에서_같이_한국어를_공부했어_.mp3`)로 교체 → 텍스트/영어 번역/오디오 경로 전부 "한글 퀘스트에서"로 되돌리고 옛 파일 삭제, reading_pool.json L5_ep01 지문도 동일하게 수정 완료.
...
1. **HQ Level 5 ep02+** — ep01 완성(삼총사의 첫 만남), 스크립트 준비되면 진행. 남은 문법 갭(-(으)ㄴ가/나 보다, -았/었더니, -(으)ㄹ까 봐, -기는요) 4개면 레벨5 하나로 충분할 듯 — ep가 쌓이면 5 단독 vs 5+6 분량 재판단.
