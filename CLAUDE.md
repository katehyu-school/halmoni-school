...
| 앱 | 현재 상태 | 다음 작업 |
|----|---------|---------|
| **HQ (nhs.html)** | L1 ep01~12 완성 ✅ / L2 ep01~12 완성 ✅ / **L1·L2 문법 annotation 툴팁 완성 ✅** / **L3 ep01~12 완성 ✅** / **L3 마감 테스트 완성 ✅** / **L4 ep01~03 완성 ✅** | L4 ep05·07 슬라이드/TTS 준비되면 구현 |
...
- 에피소드 추가 = `data/nhs/L{n}/epNN.json` 파일만 추가 (nhs.html 사이드바 배열에 1줄만 추가하면 됨)
...
- **가족별 거주지 설정 (배경 묘사로만 암시)**: 미래네 = 캘리포니아 / 리암네 = 토론토 — 마당 화초·풍경이 각 지역 기후를 반영해야 함. 새 슬라이드·배경 이미지 작업 시 이 기후 일관성 체크할 것.
...
**가족 1 (미래네)**: 엄마, 아빠, **미래 (Mirae)** (15세) 👧, **리아 (Lia)** (13세), **애라 (Aera)** (5세)

**가족 2 (리암네)**: 엄마(=리암 엄마, 리암/카요와 함께 자주 등장), 아빠, **리암 (Liam)** (13세), **카요 (Kayo / Kyle)** (11세)
...
| 10 🛏️ 할머니댁 — 미래/리아 방 | -(으)ㄴ/는 편이다 (경향) | ✅ 완성 |
...
| 02 | 🧊 미래네 부엌 | 이게 왜 여기 있지? | -(으)ㄹ 리가 없다/있다, -겠- | ✅ 완성 |
| 03 | 🏡 할머니 댁 정자 | 우주 대스타의 삶이란 | -기 마련이다★NEW, -는 법이다★NEW, -기 십상이다 | ✅ 완성 |
| 04 | 🏞️ 개천 | 여기가 왜 이렇게 좁아졌어요? | -더-★NEW, -더라고요★NEW, -던 N★NEW | 📝 분석완료 · 슬라이드/TTS 대기 |
| 05 | | 넌센스 속담 알아 맞추기 | 한국 속담, -다는 말이에요, -는 셈이다 | 📝 분석완료 · 슬라이드/TTS 대기 |
| 07 | | 죽을 지경이야 | -(으)ㄹ 지경이다★NEW, -을 정도로, -기 짝이 없다 | 📝 분석완료 · 슬라이드/TTS 대기 |

> ⚠️ **ep03 확정 사항**: -(으)ㄹ 법하다는 별도 문법카드 대신 **낱말카드**로 처리 (📖 단어 인덱스 노출 목적). 실제 자연스러운 형태는 -(으)ㄹ 법하다 / -았·었을 법하다뿐 — "것 같다"식 ㄴ/는/ㄹ 3분할은 언어적으로 성립 안 함.
> ✅ **ep03 주인공**: 할머니 = ep01(대문 장면)과 동일 인물 (`id:"grandma"`, var(--coral)). 딸 = **리암 엄마** (L1 ep11·L3 ep08에도 등장하는 기존 캐릭터, `id:"liam_mom"`, color "burgundy" — grandma와 색상 충돌 방지). scene "🏡 할머니 댁 정자" = ep01 대문과 같은 집.
> 📝 **ep04 확정 사항 (2026-07-03 분석)**: 등장인물 할머니+리암(형)+카요(동생) — 기존 세계관과 일치(L2 ep05·L3 ep01/10/11에서 이미 확립된 조합), 장면은 개천(CLAUDE.md 세계관 노트의 "올챙이 개울" 실화 추정, 선생님 확인 필요). -더-/-더라고요/-던 N 전부 L1~L4 통틀어 최초 정식 도입(★NEW) — 기존엔 `-더라도`(양보, 다른 문법)만 있었음. 스크립트 리비전: "많았더랬는데"→"많았었는데", "더 컸더랬는데"→"더 컸었는데"로 순화, "쪼그라들었는지/더 컸더랬는데" 조크 2줄(구 10~11번 라인)은 군더더기라 삭제 — 최종 10줄로 확정. 문법 커버리지: "왔던"(1줄)/"다니던"+"짧아졌더라고"(6줄)/"귀여웠던"(10줄) — 슬라이드/TTS 대기 중.
> Level 4 마감 테스트: 아직 미구현 (L1/L3처럼 에피소드 전체 완성 후 설계 예정)
...
1. **HQ L4 ep05·07** — 슬라이드/TTS 준비되면 구현 (스크립트 분석은 완료)
