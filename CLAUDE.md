...
> 📋 **2026-09-06 — 코드 변경 없이 결론만 난 것들 (다음 세션 참고용)**
> — 🔍 **색인 문제의 정체 확인**: Search Console "Why pages aren't indexed" = **`Discovered - currently not indexed` 118개** + `Alternate page with proper canonical` 1개. 이건 **구글이 주소만 알고 아직 크롤링조차 안 한 상태** — 내용이 얇아서 거부당한 게 아님(그건 `Crawled - not indexed`). 그래서 **페이지 내용을 늘리는 건 지금 효과 없음**. 점검해 보니 sitemap 122개 전부 실제 페이지(리다이렉트·404 0건), 허브(`/learn/`)에서 117개 전부 진짜 `<a>` 링크로 연결, 대문→허브→각 페이지 2클릭 — **기술적으로 할 수 있는 건 이미 다 돼 있음**. 남은 레버는 **외부 링크(페이스북·유튜브 채널에 doranchae.com 걸기)와 시간**뿐. 대문은 9/5 오후에 재크롤됨(색인 요청 반영), 파비콘도 그때 새로 읽어감.
> — ❌ **Kids에 "복습/preview 딱지" 도입 — 보류 결정**: 어휘 382개 중 이전 과에 다시 나오는 단어가 **31개(8%)뿐**이라 한 과에 딱지가 0~2개 붙는 수준 → 값어치 낮다고 판단. 문법 tier 딱지는 **4권만 데이터(core 19·preview 3·foundation 3)와 렌더러가 이미 있고**, 3권은 데이터 3개 + 렌더러에 tier 표시 자체가 없음. 확장하려면 3권 문법 31개에 선생님이 직접 tier를 지정해야 함(교육적 판단이라 자동 계산 불가). **먼저 4권 딱지에 대한 아이들 반응을 보고 결정하기로 함.**
> — 🖼️ **페이스북 페이지 개설 지원**: 프로필 이미지(한옥 크레스트 원형, 1000×1000) 2종을 폴더에 넣어 둠 — `social-profile-doranchae.png`(심볼+도란채, 추천) / `social-profile-doranchae-symbol.png`(심볼만). Gaegu 글꼴 버전은 구글 폰트가 컨테이너에서 안 받아져서 **브라우저에서 그리는 도구 `tools/fb-profile.html`**을 만들어 둠(Live Server로 열고 버튼 한 번 → PNG 저장). 페이지는 **개인 계정과 분리된 익명 페이지**로 운영, Kids 자료·손주 사진은 올리지 않기로.
> — 🧑‍🎓 **체험 계정 첫 외부 가입자 `tim`** — 9/4 저녁 가입·로그인 1회, 학습 기록은 아직 없음. admin.html의 새 "체험 계정 활동" 섹션에서 접속/학습을 구분해 볼 수 있음(30일 미접속 시 자동 삭제).
> — 💬 **사이트가 복잡하다는 외부 피드백** — 첫 화면 버튼 24개·이모지 26개로 근거 있음. 선생님 딸(전직 Meta UI 전문가·손주들 엄마)에게 자문 요청하기로 하고 카톡 문구까지 정리함(핵심 질문 2개: ①아이 옆에서 10분 보고 멈칫하는 데 3곳 ②첫 화면에서 제일 먼저 눌러야 할 버튼 하나). **답을 받으면 그때 화면 정리 착수.**
> — ⏳ **미결 1건**: 4권 2과 `책 빌리러`가 듣기 퀴즈·문장 조립·쓰기 예시 **4곳에 더** 있음. 역할극 대사만 `책을 빌리러`로 고쳤고 나머지는 그대로 — 통일할지 선생님 판단 대기.
> — ⏳ **미확인 1건**: 1·2권에도 `B1_INLINE_DATA`가 있어 3권과 같은 함정일 수 있음(1·2권 내용 수정 시 먼저 확인할 것).

> 🔴 **2026-09-06 발견·수정 (3권이 파일 대신 번들 안 사본을 읽고 있었음 — 내용 갱신이 3년째 막혀 있던 구조)**
> — 증상: 위 "이 장면에서" 작업을 마쳤는데 **선생님 화면에는 안 나옴**. 렌더러 함수를 직접 부르면 나오는데 앱 화면에는 없음 → 데이터가 다른 데서 온다는 뜻.
> — **원인**: `loadBook3Unit()`이 **번들에 박아 둔 `B3_INLINE_DATA`(1~5과, 59KB)를 fetch보다 먼저** 썼음. 그래서 `data/elem/level3/unit01~05.json`을 아무리 고쳐도 1~5과에는 **영원히 반영되지 않는 구조**였음. (4권 `loadBook4Unit`은 원래부터 fetch 우선이라 무사)
> — 😱 **그 사이 실제로 학생에게 안 보이고 있던 것**(인라인 사본 vs 파일 비교로 확인): **3권 4과 문법 "~고 싶어요" 항목이 통째로 누락**, 3권 5과 연결어미 라벨(이유) 누락, 그리고 이번에 넣은 장면 대사 전부.
> — **수정**: `loadBook3Unit`을 4권과 같은 방식(**fetch 우선 → 실패 시 인라인 폴백**)으로 통일. 인라인 데이터는 `file://`로 직접 열 때를 위한 예비용으로만 남김. 이제 `data/elem/*.json`이 3권·4권 모두의 단일 소스.
> — ✅ 검증: 브라우저에서 실제 화면 확인 — 3권 1과 장면 블록 ✅, **3권 4과 "~고 싶어요" 항목이 드디어 나타남** ✅. 재암호화 후 라운드트립 `cmp` 일치, `iv_hex` 갱신.
> — 📌 **교훈(다음 세션)**: Kids 콘텐츠를 고쳤는데 화면에 안 보이면 **"앱이 그 데이터를 어디서 읽는지"부터 확인**할 것. 번들 안에 사본(`B*_INLINE_DATA`)이 있는지 보고, 있으면 그게 이기고 있는 건 아닌지 의심. 1·2권에도 `B1_INLINE_DATA`가 있으므로 같은 함정 가능성 있음(아직 확인 안 함).
...
> — ⚠️ **순서 주의(다음에도 같음)**: 영어는 **`Liam`을 `Lia`보다 먼저** 바꿔야 함(`Lia`가 `Liam`의 앞부분이라 순서가 반대면 `Raonm` 같은 게 생김). 한글은 `올리비아`(HQ L5 인물)가 깨지지 않는지 매 파일 확인 — 실제로 13건 있었고 그대로 유지됨을 검증.
> — 🔍 **번들은 건드릴 필요 없었음** — Kids 앱을 브라우저에서 실제로 실행해 복호화된 스크립트 소스를 검사한 결과, 옛 이름 18건이 전부 **음성 파일 경로**였고 화면에 나오는 텍스트에는 0건. 즉 AES 번들 복호화·재암호화 절차 없이 끝남(데이터는 평문 JSON, 코드만 번들이라는 구조 덕분).
> — 🔊 **음성은 손댈 필요 없음 (확인함)**: mp3 18개의 파일명에 옛 이름이 남아 있지만, git 이력을 보니 **2026-07-23 커밋 `c1eed2d`에서 전부 새 이름으로 재녹음돼 in-place 덮어써진 상태**(파일명·경로는 관례대로 옛 이름 유지). 즉 소리는 이미 "리나"가 맞음 — 파일명만 옛 이름인 것이라 정상. **다음 세션에서 파일명만 보고 "재녹음 필요"로 오판하지 말 것.**
> — 📌 이번 누락이 생긴 경위: 2026-07-23 개명은 당시 콘텐츠가 들어 있던 `korean-app_v2.html`을 대상으로 했는데, `data/elem/*.json`(현재 앱이 실행 중에 fetch하는 실제 데이터)은 대상에서 빠져 있었음. 앞으로 Kids 콘텐츠를 일괄 치환할 때는 **`data/elem/*.json`이 진짜 데이터 소스**라는 점을 기억할 것(번들 안에는 코드만 있음).
> — ✅ 검증: JSON 44개 전부 파싱 통과, `node --check` 통과(dr-mobile·core.js·sw.js), 옛 이름 잔존 0건, `git diff --stat` = 23파일 +263/-262(=값 교체만).
> — 🔧 `sw.js` 캐시 **v4 → v5** (dr-mobile.html 변경 규칙).
> — ⏳ **git add/commit/push 대기 중**.

> 🔤 **2026-09-05 완료 (L5 발음 영어 병기 마무리 — L1~L5 전 레벨 완료)**
> — 아침에 L1·L2를 채운 뒤 남아 있던 L5 5항목(ep06 격음화 예외 / ep07 비음화·경음화·연음화 3건 / ep11 격음화)에 영어 문장 추가. 이제 **L1~L5 발음 항목 전부(56+34+33+36=159개) 한·영 병기 완료.**
> — 작업 경로가 이전과 다름: L5는 이미 Supabase(`nhs_content`)로 옮겨졌으므로 **파일이 아니라 DB를 직접 수정** — `jsonb_set(data,'{pronunciation,N,rule}', …)`로 해당 항목 한 줄만 갱신. 파일 편집도 git 커밋도 필요 없었음(이관 설계가 의도한 대로 동작한 첫 사례).
> — **L6 36항목은 그대로 한국어만** — "L5~L6은 한국어 우선" 기존 방침에 따른 것이라 손대지 않음. 바꾸려면 선생님 판단 필요.
> — ✅ 검증: 갱신 후 재조회에서 L3 34/34 · L4 33/33 · L5 36/36 영어 포함, 누락 0.

> 🔑 **2026-09-05 완료 (dr-mobile.html — 앱 안에서 직접 로그인)**
> — 선생님 요청: "DR-mobile에도 로그인 버튼이 하나 있어야겠어."
> — **원래 구조의 구멍**: 모바일 앱은 여태 *웹(index.html)에서 이미 로그인한 같은 브라우저의 `hq_user`를 빌려 쓰기만* 했음. 즉 **앱만 설치한 사람은 로그인할 방법이 아예 없었고**, 계속 비회원(Level 1·2)에 머물며 진도도 서버에 저장되지 않았음. 어제 가입한 체험 계정 Tim이 딱 이 상황에 걸릴 수 있었던 구조.
> — 프로필 화면(앱 첫 화면) 아래에 **로그인 상태 박스** 신설: 로그인 전엔 `🔑 로그인 · Log in` 버튼, 로그인 후엔 `👤 {이름}님으로 로그인 중` + 등급 표시(정식 회원 · 전 레벨 / 체험 계정 · Level 1·2) + 로그아웃.
> — 로그인 시트: 아이디·비밀번호 → `verify_login` **REST 직접 호출**(모바일은 supabase-js를 싣지 않는 기존 관례 유지). 성공 시 `hq_user`(localStorage+sessionStorage) + `nms_current`/`ms_current` 저장 → **웹 로그인과 완전히 같은 상태**가 됨. 웹처럼 `class_code_for_member`도 받아 저장하므로 로그인 후 Kids 가정반 카드도 열림(실패해도 로그인은 성립).
> — 로그인 성공 후 `location.reload()` — `HQM_ROLE`·`HQM_OPEN_MAX` 같은 판정값이 페이지 로드 시 const로 한 번만 계산되기 때문에 부분 갱신보다 새로고침이 확실함(잠긴 레벨·잠긴 학습 풀이 즉시 반영됨).
> — 계정이 없는 사람을 위해 시트 하단에 "웹에서 무료로 만들기 →"(index.html?login=1) 링크. 앱 안에서 가입까지 만들지는 않음 — 가입 흐름(체험 계정 하루 5개 제한 등)은 웹에 이미 있고 두 곳에 두면 갈라지므로.
> — ✅ **검증**: `node --check` 통과, 세 상태(비회원·정식 회원·체험) 렌더링 확인·페이지 에러 0건, **실제 서버 왕복 확인** — 존재하지 않는 계정으로 시도해 `verify_login`이 null을 돌려주고 "아이디나 비밀번호가 맞지 않아요."가 뜨는 것, 그리고 실패해도 기존 로그인이 덮이지 않는 것까지 확인(실제 비밀번호는 사용하지 않음).
> — 🔒 **이어서 (선생님 확인 후) — 앱 입장에 계정 필수로 변경**: 선생님이 "로그인 안 해도 버튼만 누르면 통과된다"고 발견 → 이는 버그가 아니라 웹과 같은 게스트 정책이 앱에도 적용된 것이었으나, 확인 중 **진짜 문제**가 드러남 — 진도 저장소가 로그인 전 `nms_나_*` / 후 `nms_@아이디_*`로 **서랍이 갈려서, 게스트로 공부하다 로그인하면 기록이 안 이어짐**. 앱은 검색에 안 잡히고(robots+noindex) QR·링크로만 오는 도구라 게스트 모드의 유입 효과도 없음 → **선생님 결정: 계정 필수(체험 이상)**.
> — 구현: `drHasAccount()` 문지기(허용 등급 student/teacher/admin/**trial**) → `enterHQ()`·`finishOnboard()`에서 확인, 막히면 상황별 안내와 함께 로그인 시트를 엶. 프로필 화면의 Doranchae 카드 부제도 로그아웃 상태에선 "🔒 로그인이 필요해요"로 바뀜. **게스트 등급으로 로그인한 경우**는 "로그인은 됐지만 이 등급으론 못 쓴다"는 상태를 솔직히 표시하고 [다른 계정으로 로그인]·[로그아웃]을 같이 제공(진도가 서버에 저장되지 않는 등급이라 제외).
> — **웹(doranchae.com)은 그대로 게스트 열람 유지** — "웹에서 맛보기 → 계정 만들기 → 앱은 복습용" 흐름. 두 정책이 다른 이유를 코드 주석에도 남김.
> — ✅ 네 가지 상태 전수 확인: 비회원 ❌막힘 / 게스트 등급 ❌막힘 / 체험 ✅통과 / 정식 ✅통과, 페이지 에러 0건.
> — 🔧 `sw.js` 캐시 **v3 → v4** — dr-mobile.html을 고쳤으므로 규칙대로 올림(안 올리면 기존 이용자에게 로그인 버튼 없는 옛 화면이 한 번 더 보임).
> — ⏳ **git add/commit/push 대기 중** — `dr-mobile.html`·`sw.js`.
...
> — 체험 섹션 구성: 요약 3칸(체험 계정 수 / 학습 기록 있는 사람 / 가입만 하고 아직 안 함) + 표(체험 계정 · 현재 레벨 · 가입 · **마지막 접속** · **마지막 학습** · 자동 삭제까지). **접속과 학습을 나눈 게 핵심** — 둘러보기만 하고 간 사람과 실제로 공부한 사람을 구별하려는 것. 자동 삭제까지 7일 이하면 주황색.
> — RPC 추가 없음 — 이미 있던 `admin_list_members(p_filter:'trial')`를 한 번 더 부르는 것으로 끝(같은 `Promise.all`에 합침, 새로고침 버튼 하나로 두 섹션이 같이 갱신됨). 서버 변경 0.
> — 🐛 만들면서 잡은 것: 회원 관리 표용 CSS `.member-table td:nth-child(5){width:100%}`가 새 표에도 상속돼 '가입' 칸이 한 글자씩 세로로 접혔음 → `.pg-trial-table` 전용 규칙(nowrap + 이름 칸이 남는 폭 흡수)으로 해결.
> — ✅ **검증**: `node --check` 통과, null byte 0, `</html>` 1개, `git diff --stat` = `admin.html | +87/-2`. **실제 렌더링 확인** — Playwright로 표본 데이터(tim: 가입만·기록 없음 / aaa: L1·학습 기록 있음)를 넣어 그려보고 스크린샷으로 열 정렬·색·남은 일수 계산(aaa 마지막 접속 4일 전 → 26일) 확인.
> — ⏳ **git add/commit/push 대기 중** — `admin.html` 1개 파일.
...
> — **수정**: 개별 화면마다 검사를 넣지 않고 **풀 자체를 원천에서 거르는 방식** — `_vPool()`(단어)·`_gPool()`(경어법 문법) 헬퍼 신설 후 ① 목록·검색(`_vMatch`) ② 복습 덱 기본 풀(`_sBuildDeck`) ③ '내 어휘 지도'의 분모 ④ 경어법 SRS 덱(`_gBuildDeck`의 due·fresh 양쪽)에 적용. **`GRAM_ALL`도 L3 3개·L4 2개·L6 1개가 비회원에게 나가고 있었음** — 같이 막음.
> — 레벨 칩도 에피소드 목록과 동일하게: 잠긴 레벨은 흐리게 + 🔒 표시, 누르면 안내(`vLockedMsg()`), 비회원에게는 "지금 Level 1·2 단어를 보고 계세요 · 계정 만들기" 안내 박스(`HQM_IS_MEMBER` 기준, 문구·스타일 전부 에피소드 목록 것 재사용).
> — 체험(trial) 계정도 비회원과 동일하게 L1·L2까지만 — `HQM_IS_MEMBER`가 admin/teacher/student만 포함하는 기존 규칙 그대로(웹 정책과 일치).
> — ✅ **검증**: HEAD 클린 사본에서 Python 앵커 치환(7곳 각각 count==1) → null byte 0, `</html>` 1개, `node --check` 통과, `git diff --stat` = `dr-mobile.html | 28 (+22/-6)`. **컨테이너에 실제 정적 서버를 띄우고 Playwright로 3가지 상태를 직접 실행 검증** — 비회원 555개(L1 291+L2 264)·`lvsInList=[L1,L2]` / 체험계정 555개·[L1,L2] / 정식회원 1,626개·[L1~L6], 페이지 에러 0건. 스크린샷으로 칩 🔒·안내 박스도 눈으로 확인.
> — ⚠️ **남은 한계(코드로는 못 막음)**: `data/nhs/vocab_index.json`은 여전히 **공개 정적 파일**이라 주소를 직접 치면 누구나 받을 수 있음(`honorific_grammar_index.json`도 동일). 이번 수정은 "앱 화면에서 떠먹여 주지 않게" 하는 것까지이고, 진짜로 막으려면 L3+ 에피소드처럼 **Supabase RPC로 옮겨야** 함 — 선생님 판단 필요(모바일이 supabase-js 없이 REST를 직접 쓰는 구조라 작업량은 크지 않음).
> — ⏳ **git add/commit/push 대기 중** — `dr-mobile.html` 1개 파일.

> ⌨️ **2026-09-05 완료 (자판 연습 — 맨 위에 겹받침(④⑤)까지 세분화한 음절 블록 그림 추가)**
> — 선생님 요청: Start Here › 한글 기초 › ⌨️ 자판 연습 제일 위에, 음절 블록 그림의 받침 부분을 **겹받침(4·5)으로 나눈 버전**을 넣어 한글 타이핑 순서를 알려주자.
> — `sylBlockGuide(mode)`에 **`'typing'` 모드 신설** — 기존 4개 모드(cons/vowel/batchim/syllable)는 코드·출력 그대로 두고 분기만 추가했음. 받침 줄(`3 / 1 / 4 / 3`, 두 칸 폭)을 두 칸(`3/1/4/2` + `3/2/4/3`)으로 쪼개서 ④·⑤로 번호를 매김. 46px 칸에 "Consonant"가 안 들어가서 이 두 칸만 라벨을 `Cons.`로 축약.
> — **⑤는 점선 테두리** — 겹받침일 때만 존재하는 칸이라는 걸 시각적으로 구분하려고 `cell()`에 `dashed` 파라미터(7번째, 선택)를 추가함. 기존 호출부는 인자를 안 넘기므로 `undefined`→실선 그대로, 출력 변화 0.
> — 캡션(한/영 병기): ① 첫 자음 → ② 모음(아래) → ③ 모음(옆) → ④ 받침 → ⑤ 겹받침의 둘째 자음 + 예시 `값 = ㄱ+ㅏ+ㅂ+ㅅ`, `화 = ㅎ+ㅗ+ㅏ`. **번호 순서가 실제 타이핑 순서와 정확히 일치함** — 복합모음은 아래 모음(ㅗ)을 먼저 치고 옆 모음(ㅏ)을 나중에 치므로 기존 음절 모드의 ②=아래·③=옆 번호를 그대로 써도 맞음(이미 `TP_VOWEL_COMBO`·`TP_CODA_COMBO` 로직이 그렇게 조립함).
> — 삽입 위치: `buildTyping()`의 `</style>` 직후, 안내문(`basics-intro`)보다 위 — 즉 화면 제일 위.
> — ✅ **검증**: HEAD 클린 사본에서 Python 앵커 치환(4곳 각각 count==1 확인) → null byte 0, `</html>` 1개, 인라인 `<script>` 블록 `node --check` 통과, `git diff --stat` = `nhs.html | 18 (+13/-5)`. **Playwright로 실제 렌더링 스크린샷을 찍어 typing/syllable/batchim 세 모드를 나란히 비교** — 신규 ④⑤ 분할·점선 표시 정상, 기존 두 모드 출력은 이전과 동일함을 눈으로 확인.
> — ⏳ **git add/commit/push 대기 중** — `nhs.html` 1개 파일.
> — 🧹 참고: 검증용으로 잠깐 만든 `_preview_tmp.html`은 이 마운트에서 삭제 권한이 없어 `_to_delete/`로 옮겨둠(선생님이 그 폴더째 지우시면 됨).

> 🔤 **2026-09-05 완료 (L1·L2 발음 탭 영어 보충 — 전 레벨 병기 통일)**
> — 선생님 발견: 에피소드 `<발음>` 탭에 영어 설명이 없다 → 전 레벨 전수 확인 결과 **L1·L2만 한국어 단독**이었음(L3 34개·L4 33개는 이미 전부 병기, L5는 4개만 누락, L6는 전부 한국어 — L6는 "L5~L6 문법 설명 한국어 우선" 방침과 맞물려 있어 이번엔 손대지 않음).
> — **코드 변경 없음** — 렌더러(`buildPron()`)는 `item.rule`/`item.tip` 문자열을 그대로 출력하므로, L3·L4가 이미 쓰고 있는 **"한국어 설명 · English explanation"(가운뎃점 구분) 관례**를 L1·L2 데이터에 그대로 맞추기만 하면 됨. 새 필드(`rule_en` 등)를 만들지 않은 이유도 이것 — 필드를 새로 만들면 렌더러를 고쳐야 하고, 그러면 L3~L6와 표기 방식이 갈림.
> — 반영 범위: L1·L2 **발음 항목 56개 전부** — `rule` 51건에 영어 문장 추가(이미 병기돼 있던 L1 ep07·ep08·ep09·ep10 5건은 건드리지 않음) + `tip` 6건(L1 ep03·ep04×2·ep06·ep11×2) 추가. 표 안의 "규칙 · Rule" 칸(예: `ㅂ+ㅎ → ㅍ (격음화)`)은 L3·L4도 한국어 약식 표기 그대로라 통일성 위해 손대지 않음.
> — 작업 방식: JSON을 파싱 후 재출력하면 들여쓰기가 통째로 재포맷돼 diff가 폭발하므로(2026-08-03 교훈), **원본 문자열을 `json.dumps` 형태로 정확히 1회 매칭되는지 확인한 뒤 텍스트 치환** → `os.replace`로 원자적 저장.
> — ✅ **검증**: 20개 파일 전부 `json.load` 통과, null byte 0, `git diff --stat` = **20 files / +57 / -57**(추가·삭제 수가 정확히 일치 = 값 교체만 일어남, 구조 변경 0), 재검사 결과 L1·L2 발음 56개 항목 중 영어 누락 0개·tip 14개 중 누락 0개.
> — ⏳ **git add/commit/push 대기 중** — `data/nhs/L1/*.json`·`data/nhs/L2/*.json` 20개 파일. **GitHub에 반영해야 서버(doranchae.com)에 적용됨.**
> — 🧑‍🎓 **덤 — 체험계정 첫 외부 가입자 확인**: `tim`(표시명 Tim, role=trial) 2026-09-04 18:19(토론토) 셀프 가입 + 즉시 로그인 1회. 지금까지의 trial(`aaa`)은 테스트 계정이었으므로 **실제 학습자로는 처음**. 정식 학생으로 올리려면 admin.html 회원 관리에서 등급을 student로 변경(체험은 30일 미접속 시 자동 삭제되므로 방치하면 사라짐).
> — 🔜 **선생님 확인 대기**: L5 발음 4항목(ep06 pron_ep06_l5_03 / ep07 pron_ep07_l5_01·02·03)만 영어가 없음 — 원하시면 같은 방식으로 채울 수 있으나 L3~L6는 Supabase `nhs_content`에 있어 DB 수정이 필요함.
...
> — 기존엔 "매일 단어 복습"(Step 3) 바로 다음이 "레벨 마감 테스트"(옛 Step 4)였음. 그 사이에 "🔁 레벨 복습" 스텝을 새로 넣고(새 Step 4), 마감 테스트를 Step 5로 밀어냄. 내용: 12편 다 안 끝나도 사이드바 맨 아래에서 아무 때나 열 수 있다는 것, 마감 테스트 전에 한 번 훑어보길 권장, 어휘 퀴즈 탭 + (2026-08-31에 새로 생긴) 문법 SRS 탭 둘 다 간단히 설명.
> **2) 숨은 기능 표 — "색인" 행에 북마크 안내 한 줄 추가**
> — 가입 회원(student/teacher/admin)이 색인의 🔖 버튼으로 북마크할 수 있고, 그 북마크가 각각 복습 플래시카드 · 레벨 복습의 문법 SRS 큐에 자동으로 들어간다는 설명 추가.
> **3) 어휘문법_복습메카니즘_총정리.md 확인**
> — 선생님이 최신 작업(모바일 진도 pull, 웹 문법 SRS)이 반영 안 된 것 같다고 걱정하셨는데, 확인해보니 이미 다 반영돼 있었음(알려진 한계 1·2번 모두 "해결(2026-08-31)"로 표시됨, git status로도 이 파일은 clean/커밋됨 확인) — 추가 수정 없음.
> **4) (도메인) hangeulquest.com → doranchae.com 리다이렉트 버그 2건 수정 (Cloudflare)**
> — Search Console 인덱싱 조사 중 발견: Cloudflare Page Rule이 301로 설정돼 있는데도 실제로는 302로 나가는 문제 + 어떤 경로로 들어와도 doranchae.com **루트로만** 리다이렉트되는 문제(원래 경로가 사라짐). 원인은 URL 패턴 `*hangeulquest.com/*`에 와일드카드가 2개라 `$1`(도메인 앞부분, 항상 빈 값)과 `$2`(실제 경로)가 따로 생기는데, 목적지가 `$1`을 참조하고 있었던 것. 목적지를 `https://doranchae.com/$2`로 수정 + 302→301→302→301로 두 번 저장해서 강제 재배포 → 이제 경로 유지되고 301로 정상 동작 확인(WebFetch로 검증).
> — Search Console 인덱싱 자체(118개 "Discovered - currently not indexed")는 이 리다이렉트 문제가 원인이 아니라 아직 크롤링 안 된 정상적인 백로그(검증 시작 8/30)로 확인됨 — 1~2주 뒤 재확인 권장.
> — ✅ **검증**: `nhs.html` null byte 0, `</html>` 1개, 인라인 `<script>` 블록(전체 코드 한 블록) `node --check` 통과, `git diff` 리뷰 완료(순수 추가 3곳). Cloudflare 쪽은 WebFetch로 실사이트 재확인.
> — ⏳ **git add/commit/push 대기 중**(선생님이 직접 커밋). Cloudflare는 코드가 아니라 대시보드 설정이라 커밋 대상 아님.
...
> — 수정: Step 4를 `rm-s3` 래퍼 안으로 옮겨서 '3. 매일 복습' 탭과 함께 보이고 숨겨지게 함(매일 복습 → 마감 테스트로 이어지는 흐름이라 묶는 게 자연스러움). 다른 탭에서는 더 이상 끼어들지 않음.
> — ✅ **검증**: null byte 0, `</html>` 1개, 인라인 `<script>` 블록 `node --check` 통과, `git diff --stat` = `nhs.html | 3 (+1/-2)`.
...
> — **최종 방식으로 재구현**: 5개 버튼을 탭처럼 동작하게 함 — 클릭하면 스크롤 없이 **버튼 바로 아래에 해당 섹션만** 나타나고 다른 섹션은 숨겨짐(`_rmTabBtn()`/`_rmTabShow()`, `RM_TAB_IDS`). 기본값은 '0. 한글 기초'가 열려 있음. Step 2의 8단계 상세, 📦 모르면 그냥 지나치는 것들(12개 표)는 아코디언 없이 **항상 펼쳐진 상태**로 되돌림 — `_rmToggleBtn()`/`_rmToggle()`는 완전히 제거. Step 4(마감테스트)·QR·학습자료·주간리듬은 이전과 동일하게 버튼과 무관하게 항상 표시(5개 버튼 목록에 없었으므로).
> — 작업 방식: 대용량 파일 손상 이력(위 "작업 습관" 참고) 때문에 Edit 툴 대신 Python 앵커 치환(각 조각 고유성 count==1 확인) + `os.replace`로 원자적 저장.
> — ✅ **검증**: null byte 0, `</html>` 1개, 인라인 `<script>` 블록(108~7485행) `node --check` 통과, 잔존 참조(`_rmToggle`·`_rmJump`·`rm-s2-detail`·`rm-gems-detail`) 0건, `git diff --stat` = `nhs.html | 63 (+30/-33)`.
> — ⏳ **git add/commit/push 대기 중** — 로컬 파일만 수정됨(`nhs.html`). **GitHub에 반영해야 서버(doranchae.com)에 적용됨.**
...
> — **`tools/build_static_pages.py`에 `REDIRECTS` 딕셔너리 + `redirect_page()` 함수로 통합**(수작업 파일로 옆에 만들지 않음) — 이유: `build()`가 매 실행마다 `learn/`을 통째로 지우고 다시 만들기 때문에, 스크립트 밖에서 손으로 만든 리다이렉트 파일은 다음 실행에서 그냥 사라짐. `REDIRECTS`에 등록해두면 재실행해도 항상 같이 재생성됨. 리다이렉트 페이지는 `hq-mobile.html`(예전에 hq→dr 주소 바꿀 때 쓴 것과 동일 패턴) — `<link rel=canonical>`+`<meta http-equiv=refresh content="0;url=...">`+`location.replace()`+수동 링크, `<meta name=robots content=noindex,follow>`(리다이렉트 스텁 자체는 색인 안 되고 canonical이 검색엔진에 진짜 주소를 알려줌). sitemap.xml에는 넣지 않음(진짜 콘텐츠 페이지가 아니므로).
> — 정적 사이트라 서버 차원의 진짜 301은 불가능 — 이 meta-refresh+JS 방식이 이 프로젝트의 표준 리다이렉트 패턴(`hq-mobile.html`에서 이미 검증됨).
> — ✅ **검증**: 8개 파일 전부 `HTMLParser`로 태그 균형 확인. Playwright로 3개 샘플(문법→문법 1개, 문법→에피소드 2개) 실제 네비게이션 확인 — `location.replace()`가 실제로 새 URL로 이동시키는지 최종 `page.url`로 대조 확인.
> — 배포: 스크립트를 `--levels L1 L2`로 재실행(315개 = 기존 307개 + 리다이렉트 8개) → zip으로 `device_commit_files`, `device_bash`로 압축 해제. 기기 파일 개수 315개로 컨테이너와 일치 확인.
> — ⏳ **git add/commit/push 대기 중**.

> 🛠️ **2026-08-27 완료 (`tools/build_static_pages.py` 재작성 — Level 3+ 콘텐츠를 지금 미리 써놓고 나중에 잠금만 풀 수 있게 함 + 검색엔진 노출도 스크립트 한 번 실행으로 처리, 바로 아래 "📝 참고 메모"를 대체함)**
> — 선생님 질문 2개: ① "나중에 '학습 자료 Level3+ 내용 보충해줘'라고 하면 클로드가 알아들을까?" ② "아예 지금 다 써놓고 나중에 잠금만 풀리게 하면? 그리고 Level3+도 검색엔진에 잡히게 하려면 뭔가 작업이 필요해?"
> — **결론**: 콘텐츠 소스(`data/nhs/L3~L6/ep*.json`의 grammar 배열)는 이미 다 있고, 정적 페이지 생성기(`tools/build_static_pages.py`)도 이미 있었으므로, "지금 다 써놓기"와 "필요할 때 즉석에서 생성하기"가 사실상 같은 작업량 — 생성기가 "열린 레벨 vs 잠긴 레벨"을 한 번에 처리하도록 재작성하면, 미리 써놓을 필요 없이 **`--levels` 옵션만 바꿔서 다시 실행하면 그게 곧 "내용 보충 + 잠금 해제 + sitemap 등록"이 한 번에 끝나는 구조**로 만들 수 있다고 판단해 이 방향으로 진행.
> — **스크립트 변경 내용**: `page()` 함수에 `locked` 파라미터 추가(True면 `<meta name='robots' content='noindex,follow'>` 자동 삽입 + JSON-LD `isAccessibleForFree:false`) + `extra_head`/`extra_body_end`(허브 탭 UI용 CSS/JS 주입). `locked_grammar_body()`/`locked_episode_body()` 신설 — 기존 수작업 placeholder에 있던 결함(내부 문법 카드가 여전히 `<a href>`라 클릭되던 문제)까지 고쳐서 `<div class='card lockcard'>`(클릭 불가)로 렌더링. `build(open_levels)`를 항상 `ALL_LEVELS`(L1~L6) 고정 순서로 순회하도록 변경 — 어느 레벨이 열려있든 잠겨있든 파일명(slug)이 항상 똑같이 나와서, 나중에 잠금만 풀어도 URL이 안 바뀜(SEO 안전). 열린 레벨은 진짜 콘텐츠+sitemap 등록, 잠긴 레벨은 placeholder+noindex — 같은 스크립트 한 번 실행으로 전부 처리. 허브 페이지(`learn/index.html`)의 탭바·카드도 이제 6개 레벨 전부 프로그램적으로 생성(레벨 열림 여부에 따라 `<a>` 카드 or `<div class='card lockcard'>` 카드).
> — **결과적으로 선생님 질문에 대한 답**: ① 다음 세션이 "Level3+ 내용 보충해줘"를 받으면 → `python3 tools/build_static_pages.py --levels L1 L2 L3`(원하는 레벨까지)를 실행하면 끝. 새로 집필하거나 수작업으로 lockcard를 되돌리는 작업 불필요 — 스크립트가 콘텐츠 채우기+링크 활성화+탭 아이콘 변경+sitemap 등록을 전부 자동으로 함. ② "지금 다 써놓기"는 사실상 불필요해짐 — 스크립트가 있으니 "필요할 때 생성"과 "미리 생성"이 비용 차이가 거의 없고, 미리 써두면 오히려 관리 부담(콘텐츠 변경 시 재생성 필요)만 늘어남. 검색엔진 노출도 별도 작업이 필요 없고 `--levels`에 포함시키기만 하면 sitemap.xml에 자동 등록됨.
> — **배포 중 발견한 잔재 정리**: 컨테이너에서 재생성한 `learn/`(307개 HTML)을 zip으로 압축해 기기에 전송·해제했더니 기기에 420개가 있어서 113개 차이 발견 — 예전(스크립트로 저장 안 된, 유실된 버전)의 생성기가 남긴 고아 파일들이었음. 제목 매칭으로 확인한 결과 105개는 L3~6 placeholder(이미 잠겨있고 링크도 없어 위험 없음), 8개는 L1·L2 파일인데 이후 JSON 콘텐츠가 수정되면서 slug가 바뀐 것들(이 세션에서 만든 문제 아님, 예전부터 있던 static-page staleness). `device_request_delete_permission`으로 삭제 권한 받아 113개 전부 삭제 → 재확인 중 기기 308개 vs 컨테이너 307개로 1개 차이 추가 발견 → `learn/grammar/l6-ep11-i-saw-found-that-reporting-a-past-observation.html`(구버전 slug, 신버전은 `l6-ep11-i-saw-found-reporting-past.html`로 대체됨) 1개를 마저 삭제해서 정확히 307개로 일치시킴. 최종 배포 완전히 정리됨.
> — **(해결됨 — 바로 위 항목 참고)** slug가 바뀐 L1·L2 8개 URL: 당시엔 2개만 새 slug로 매칭됐었는데, 바로 위 항목(같은 날 후속 작업)에서 git 히스토리로 다시 확인해 실제로는 5개가 매칭되고 3개는 대응 페이지가 없어 에피소드로 보내는 것으로 정리, 8개 전부 리다이렉트 스텁 배포 완료.
> — ✅ **검증**: 재작성한 스크립트로 `--levels L1 L2` 실행한 결과물을 Playwright 스크린샷으로 예전 수작업 버전과 시각적으로 비교(동일 확인). 최종 기기/컨테이너 파일 개수 307개로 정확히 일치 확인.
> — 배포: `device_commit_files`(zip 방식)로 `learn/` 전체·`sitemap.xml`·`robots.txt`·`tools/build_static_pages.py`를 `C:\Users\kateh\Desktop\halmoni-school`에 반영 완료. 잔재 파일 114개는 `device_bash`로 직접 삭제 완료.
> — ⏳ **git add/commit/push 대기 중**.

> 📝 **(위 항목으로 대체됨 — 아래는 이전 접근 방식의 기록, 참고용으로만 남김)** 나중에 "학습 자료(learn) Level 3 이상 내용 보충해줘" 요청 시 예전엔 4단계 수작업 체크리스트(lockcard→`<a>` 복원, "🔒 준비 중" 태그·안내문 삭제, 탭 🔒 이모지 제거, sitemap.xml에 URL 추가)를 안내했었으나, 지금은 `tools/build_static_pages.py --levels L1 L2 L3 ...`(원하는 레벨까지) 한 번 실행이 이 4단계를 전부 대신함. 레벨 일부만 먼저 공개하고 싶으면(예: Level 3만) "몇 레벨까지"인지 확인 후 그 레벨들만 `--levels`에 포함시켜 실행.

> 🔒 **2026-08-27 완료 (learn/ — 잠긴 레벨 카드 클릭 자체를 차단, 바로 위 항목의 후속)**
> — 탭으로 재구성한 뒤 선생님이 "Level 3 이상은 클릭도 안 되게 해달라"고 요청 — 지금까지는 `.lockcard`가 흐리게 스타일만 됐지 실제로는 여전히 `<a href>`라서 눌러보면 "🔒 준비 중" 플레이스홀더 페이지로 이동하고, 그 페이지 안의 다른 문법 카드도 또 클릭돼서 "뭔가 열리고 또 열리는" 산만한 경험이었음.
> — 잠긴 카드 190개 전부 `<a class='card lockcard' href='...'>`에서 `<div class='card lockcard'>`로 교체(href 자체를 없앰) + CSS에 `pointer-events:none`, hover 시 teal 테두리 강조 취소(`cursor:default`) 추가 — 이제 시각적으로도 완전히 비활성 상태로 보이고 클릭해도 아무 반응 없음. Level 1·2 카드는 그대로 `<a>`라 정상 작동.
> — 참고: `learn/episode/l3~l6-*.html`·`learn/grammar/l3~l6-*.html` 개별 플레이스홀더 파일 자체는 그대로 디스크에 남아있음(내용 없는 빈 페이지라 위험 없음) — 이제 사이트 어디에도 링크가 없어서 크롤러나 사용자가 우연히 도달할 경로도 사실상 사라짐.
> — ✅ **검증**: Playwright로 잠긴 카드가 `<div>`이고 `href` 속성이 없는지, `pointer-events`가 `none`인지 확인 + 강제 클릭해도 URL이 안 바뀌는지 확인. Level 1 카드는 여전히 `<a>`로 정상 작동하는지 대조 확인.
> — 배포: `device_commit_files`로 `learn/index.html`에 반영 완료.
> — ⏳ **git add/commit/push 대기 중**.

> 📖 **2026-08-27 완료 (learn/ 공개 문법 사이트 — 레벨별 탭으로 재구성, 잠금 상태 확인)**
> — 선생님이 "학습 자료(learn)를 레벨별로 탭을 만들면 어떨까? 여기도 Level 3부터는 잠금 걸어놓고" 요청. `learn/`은 nhs.html 앱과 별개의 **완전 정적 공개 사이트**(로그인 개념 자체가 없음, doranchae.com/learn/)로, 헷갈리는 문법 하나 빨리 찾아보거나 링크로 공유할 때 쓰라고 만든 무료 문법 페이지 모음.
> — **먼저 실제로 유출 위험이 있는지부터 점검**: `learn/episode/l3~l6-*.html`, `learn/grammar/l3~l6-*.html` 파일이 이미 git에 커밋·푸시까지 되어 있는 걸 확인하고 처음엔 "이미 라이브로 새고 있나?" 긴장했는데, 실제 파일을 열어보니 각 페이지 자체가 **제목만 있고 본문은 처음부터 "🔒 아직 공개되지 않았어요" 플레이스홀더**로 만들어져 있었음(진짜 문법 설명은 아예 안 들어있음) — 이미 콘텐츠 차원에서는 안전하게 설계돼 있었던 것. `sitemap.xml`에도 Level 1·2 경로만 등록돼 있어 검색엔진에도 안 노출됨. 즉 이번 요청은 보안 버그 수정이 아니라 **UI 재구성** 작업.
> — `learn/index.html`(레벨 1~6 전체를 한 페이지에 세로로 쭉 나열하던 441줄짜리 정적 페이지)을 Level 1~6 **탭 UI**로 재구성: 상단에 `Level 1 | Level 2 | Level 3 🔒 | Level 4 🔒 | Level 5 🔒 | Level 6 🔒` 탭바 추가, 각 레벨의 (에피소드 카드+문법 카드) 블록을 `<section class="lvl-panel" data-lvl="N">`으로 감싸 JS `showLvl(n)`으로 토글(기본은 Level 1만 표시). 기존 `.lockcard`(흐린 카드+🔒 접두 스타일)는 그대로 유지 — 클릭하면 이미 있던 "준비 중" 플레이스홀더 페이지로 감. 순수 정적 사이트라 실제 접근 자체를 막는 서버 로직은 없고, 애초에 콘텐츠가 없으니 막을 것도 없는 구조.
> — ✅ **검증**: Python `HTMLParser`로 태그 균형 확인, 레벨 섹션 6개·카드(일반 119 + lockcard 190) 개수가 원본과 동일한지 확인(중복/누락 없음). Playwright로 Level 1 기본 표시 확인 + Level 3 탭 클릭 시 해당 패널만 보이는지(`getComputedStyle().display`) 확인, 스크린샷으로 시각 검증.
> — 배포: `device_commit_files`로 `C:\Users\kateh\Desktop\halmoni-school\learn\index.html`에 직접 반영 완료.
> — ⏳ **git add/commit/push 대기 중**.
> — 🤔 **선생님께 확인 필요(다음에 답변 받으면 처리)**: 선생님이 "빠른 참고(Quick Reference)에 있는 자료가 공들여 만든 거라 다른 사이트에서 못 볼 깊이감이 있다"고 짚어주심 — 맞는 지적, 이전에 "레벨과 무관한 무료 콘텐츠라 잠글 필요 없다"고 판단한 근거(Basics/Start Here에서 이미 공짜로 보이는 내용과 같아서 잠가도 유료 콘텐츠 보호 효과가 없음)는 여전히 유효하지만, **"콘텐츠 자체가 무료 티어인가"와 "이 도구(항상 어디서든 바로 열리는 편의 기능)를 회원 전용 혜택으로 만들지"는 별개의 선택**이라 선생님께 직접 여쭤봐야 함 — 그대로 열어둘지(비회원 대상 quality 시연/전환 유도 효과) vs 로그인해야만 열리게 바꿀지(회원 혜택 강화). 다음 세션에서 먼저 확인.

> 🔴 **2026-08-27 완료 (🚨 보안 — 색인(Index) 통한 레벨락 우회 발견/수정, 바로 위 항목의 후속)**
> — 바로 아래 항목에서 `_lvNum` 우회 버그를 고친 직후, 선생님이 곧바로 재발견: "Start Here로 들어가서 색인을 통하면 어떤 레벨이든 갈 수 있어." → 확인해보니 진짜 새로운 우회 경로였음. 잠금 검사(`setLevel()`)는 **상단 레벨 탭 버튼 클릭**에만 걸려 있었고, 색인(Index) 모달의 단어/문법/발음 배지 클릭(`loadEpAndTab()`→`_doLoadEp()`)이나 오답노트의 "복습하러 가기" 같은 **다른 진입 경로는 애초에 잠금 검사를 거치지도 않는 구조**였음 — 회귀가 아니라 원래 설계 자체에 있던 구멍.
> — **근본 수정**: 개별 진입 경로마다 잠금 검사를 추가하는 대신, 모든 에피소드 로드가 결국 거치는 단일 함수 `_doLoadEp()` 맨 앞에 잠금 검사를 넣음(level 문자열에서 숫자 추출해 `NHS_OPEN_MAX`와 비교) — 이제 색인·오답노트·URL 딥링크·콘솔에서 직접 함수 호출 등 어떤 경로로 와도 예외 없이 막힘. Playwright로 `loadEpAndTab('L4_ep02','vocab')`를 직접 호출해 비회원은 alert와 함께 차단, 회원(localStorage `hq_user` role student)은 정상 진입되는 것 확인.
> — **콘텐츠 자체 유출도 발견해 같이 수정**: 잠금 검사를 고쳐도, 색인 모달은 애초에 **모든 레벨(3~6 포함)의 실제 단어 뜻·문법 설명·예문을 검색 결과에 그대로 노출**하고 있었음(클릭해서 들어가는 것만 막혔지, 목록 자체가 이미 유료 콘텐츠 유출). `renderIdxBody()`의 단어/문법/발음 3개 탭이 공유하는 `allEps` 수집 시 레벨 필터를 추가(`ep.level`이 `NHS_OPEN_MAX` 이하인 것만 포함), 별도 데이터소스인 `GLOSSARY_POOL`(실생활 어휘, 키가 `L4_ep02` 형식)도 똑같이 놓쳐서 따로 필터 추가. 필수낱말카드(essential)·Start Here 소속 콘텐츠는 원래도 레벨 무관 무료라 필터 대상 아님.
> — **빠른 참고(Quick Reference)는 잠글 필요 없음으로 판단**: 선생님이 "빠른 참고와 색인 모두 잠글까?" 질문 → 코드 확인 결과 빠른 참고(인사·의문사·호칭·숫자·동사활용·접속부사·띄어쓰기)는 `EPISODE_DATA`/`loadEp` 등을 전혀 참조하지 않는 **정적 하드코딩 콘텐츠**(Start Here 사이드바에서 이미 무료로 보던 것과 동일)라 레벨별 유료 콘텐츠 유출이 없음 — 잠그면 오히려 무료 사용자 경험만 줄어드는 것이라 그대로 둠. 색인만 위처럼 필터링.
> — 알려진 소소한 잔여 이슈(보안 아님, 필요시 다음에): 회원이 색인을 통해 잠긴 레벨(L4 등)로 바로 들어가면 콘텐츠는 정상 로드되지만 상단 레벨 네비 active 표시는 "Start Here"에 그대로 남아있음(`loadEpAndTab()`이 `setLevel()`을 안 거쳐서) — 이전에 고친 Road Map 케이스와 같은 종류의 화면 표시 문제, 접근 제어와는 무관.
> — ✅ **검증**: `node --check` 통과. Playwright로 비회원 기준 색인 3개 탭(단어/문법/발음) 배지가 전부 L1·L2(+필수낱말)만 나오는지 확인, 회원 기준으로는 L1~L6 전체가 나오는지 대조 확인. 비회원이 `loadEpAndTab('L4_ep02', ...)`를 직접 호출해도 alert 뜨고 차단되는지 확인.
> — 배포: `device_commit_files`로 `C:\Users\kateh\Desktop\halmoni-school\nhs.html`에 직접 반영 완료.
> — ⏳ **git add/commit/push 대기 중**.
...
> — **새 세션 시작 시 반드시 확인**: 컴퓨터(디바이스)가 연결되어 있으면, 파일 수정 후 SendUserFile로 채팅에 올리는 것에서 끝내지 말고 `device_commit_files`로 실제 프로젝트 폴더(`C:\Users\kateh\Desktop\halmoni-school`)에도 반드시 덮어쓸 것. 연결 여부가 세션 중간에 바뀔 수 있으니, 작업 시작할 때뿐 아니라 전달 직전에도 재확인하는 게 안전함.

> 🎯 **2026-08-24 완료 (자모 획순 — 표식 위치 2차 재보정, 실제 렌더링 기준으로 다시 측정)**
> — 1차 재보정(바로 아래 항목) 후에도 선생님 피드백: "아까랑 차이가 없는 것 같은데?" — 실제로 화면 확인해보니 여전히 번호가 글자 위에서 떨어져 있었음.
> — 진짜 원인: 1차 보정 때 각 글자의 실제 위치를 `<canvas>` + `fillText()`로 측정했는데, 이 방식은 브라우저가 실제로 `.stroke-ref-char`(flex 중앙정렬 div)로 글자를 그리는 방식과 다르게 글자 위치를 계산함(캔버스 텍스트 기준선 방식과 flex 중앙정렬 방식의 차이) — 그래서 측정값 자체가 실제 화면과 안 맞았음.
> — 이번엔 실제 프로덕션 `core/nhs.css` + 웹폰트를 Playwright로 그대로 렌더링해서 스크린샷을 찍고, 그 픽셀에서 직접 글자의 검은 영역(실제 잉크) 경계를 찾아 좌표를 다시 계산 — 이번엔 실제 화면과 100% 같은 방식으로 측정한 것이라 정확함.
> — 24개 전부 다시 계산, 특히 여러 획이 세로로 붙어있는 ㄹ·ㅁ·ㅂ·ㅌ·ㅍ은 번호 원(circle)들이 서로 겹쳐서 숫자가 안 보이는 문제도 발견 → 번호 크기를 14px→10px로 줄이고 겹치는 획들은 가로로 살짝 벌려서 배치.
> — ㅓ·ㅕ 데이터에 있던 순서 버그도 이번에 같이 발견해 수정: 세로획이 마지막(3번)으로 밀려있던 것을 1번으로 정정(세로획을 먼저 긋고 가로 삐침을 나중에 긋는 게 맞음).
> — ✅ **검증**: `node --check` 통과, CSS 중괄호 711/711, null byte 0/`</html>` 1개. Playwright로 24개 글자 전체 스크린샷 재확인 — 모든 번호가 실제 획 위에 정확히 놓이고 서로 겹쳐 안 보이는 경우 없음을 하나하나 확대해서 확인함.
> — ⏳ **git add/commit/push 대기 중**.

> 🩹 **2026-08-24 완료 (자모 획순 — 표식 위치 1차 재보정 시도 + ㅇ/ㅎ 화살표 방향 수정 + 기본 탭 변경, 이후 2차 보정으로 대체됨)**
> — 선생님 피드백: "'ㅇ'(시계 반대 방향)만 빼고 순서가 맞는 것 같은데 번호 붙인 표식하고 글자 사이즈가 안 맞아서.."
> — 원인(당시 진단, 불완전했음): 획순 번호·화살표 좌표를 만들 때 명조체 글자가 70×70px 칸 안에서 대략 어디쯤 그려질지 눈대중으로 가정했는데, 실제로는 그보다 훨씬 작고 중앙에 몰려서 그려짐 → 번호가 실제 획에서 떨어진 위치에 떠 보이는 문제.
> — Playwright로 실제 나눔명조 폰트를 70×70 캔버스에 그려서 각 글자(24개 전부)의 실제 픽셀 범위(bounding box)를 측정하는 보정 도구를 만들어 정확한 좌표를 다시 계산함. 글자가 칸을 더 채워 보이도록 글자 크기도 40px→52px로 키움.
> — ㅇ·ㅎ의 동그라미 획 화살표를 시계방향(↻)에서 시계 반대 방향(↺)으로 수정.
> — "글쓰기 연습" 메뉴 진입 시 기본으로 열리는 탭을 "글자 하나씩"에서 "🔤 자모 획순"으로 변경(선생님 요청).
> — ✅ **검증**: `node --check` 통과, CSS 중괄호 711/711 유지, null byte 0 / `</html>` 1개. Playwright로 24개 글자 전체 렌더링 스크린샷 재확인 — 번호가 실제 획 위에 정확히 겹쳐 보임(ㅅ·ㅇ·ㅈ·ㅊ·ㅎ 등 까다로운 글자 개별 확대 확인 포함).
> — ⏳ **git add/commit/push 대기 중**.

> 🩹 **2026-08-24 완료 (배포 실수 수정 — 자모 획순 탭에 core/nhs.css 전달 누락)**
> — 바로 아래 "🔤 자모 획순" 항목 작업 후 `nhs.html`+`CLAUDE.md`만 전달하고 `core/nhs.css`를 빠뜨림 — 그리드가 세로로 다 풀어져 쌓이고 번호·화살표가 겹치지 않고 그냥 글자 옆에 텍스트로 흘러나오는 것으로 확인됨(선생님 스크린샷). `core/nhs.css`를 다시 확인해보니 `.stroke-*` 규칙 자체가 파일에 없었음 — 재확인 결과 이 회차에서 CSS 변경분만 전달이 안 된 것이었고, 이전 회차들(pron-* 등)의 CSS는 전부 정상 반영돼 있었음(다행히 피해 범위는 이번 한 건).
> — `.stroke-grid`/`.stroke-row`/`.stroke-cell`/`.stroke-cell.stroke-ref`/`.stroke-ref-char`/`.stroke-guide-text`/`.stroke-canvas`/`.stroke-mark`/`.stroke-arrow` 규칙을 `core/nhs.css`에 다시 추가하고 이번엔 실제로 전달·반영함.
> — ✅ **검증**: CSS 중괄호 711/711. Playwright로 실제 렌더링 스크린샷 재확인 — 6칸 가로 배치, 번호+화살표가 글자 위에 정확히 겹쳐 보임 확인.
> — ⏳ **git add/commit/push 대기 중**.

> 🔤 **2026-08-24 완료 (글쓰기 연습 — "🔤 자모 획순" 신규 탭: 실제 획순 학습지 형식)**
> — 배경: "Start Here" 기본 진입점 논의 중 선생님이 "글쓰기 연습에서 오른쪽 옆 샘플 글자에 획순도 보이게 할 수 있나?"라고 질문 → 실제 한글 학습지(번호+화살표 있는 견본 1칸 + 점점 흐려지는 따라쓰기 칸들 + 빈칸)와 같은 형식을 원하시는 걸 확인. AskUserQuestion으로 범위 확정: ① 새 탭으로 독립 추가 ② 기본자음 14+기본모음 10 (총 24자) ③ 참고 그림이 아니라 실제로 손으로 쓸 수 있는 캔버스.
> — **획순 데이터 검증**: 웹 검색 2개 소스로 초안을 잡은 뒤, 선생님이 실제 학습지 사진을 보내주시며 여러 차례 정정 — 최종 규칙은 "펜을 안 떼고 이어서 쓰는 꺾임(ㄱ·ㄴ 모양 코너)은 1획, 네모(ㅁㅂㅍ)처럼 변이 여러 개 만나는 건 변마다 1획". 특히 **ㅈ/ㅊ은 "위가로+왼쪽삐침이 이어진 1획" + "그 1획 중간에서 갈라지는 오른쪽삐침 1획"**이라는 중요한 정정이 있었음 — 선생님 설명: 이게 바로 이전 세션에서 글쓰기 샘플 글자에 나눔명조 폰트를 적용했던 이유. 고딕체는 ㅅ·ㅈ·ㅊ의 다리 두 개가 바(가로선)에서 따로 갈라져 나온 것처럼 보여서, 아이들이 획순을 오해하고 따라 그리기 쉬움. 최종 데이터: ㄱ1·ㄴ1·ㄷ2·ㄹ3·ㅁ3·ㅂ4·ㅅ2·ㅇ1·ㅈ2·ㅊ3·ㅋ2·ㅌ3·ㅍ4·ㅎ3획(자음), 모음은 전부 직선이라 이견 없이 표준안 그대로(ㅏ2·ㅑ3·ㅓ2·ㅕ3·ㅗ2·ㅛ3·ㅜ2·ㅠ3·ㅡ1·ㅣ1획).
> — `STROKE_ORDER`(24자 × 획별 {x,y,화살표} 좌표, % 기준) + `STROKE_LETTERS`(순서 배열) 신설. `strokeMarksHtml()`이 번호 원+화살표를 글자 위에 절대좌표로 겹쳐 그림. `buildStrokeOrder()`가 24행 그리드 생성 — 각 행: [①견본(번호+화살표 오버레이)] + [흐려지는 따라쓰기 3칸(투명도 0.55/0.32/0.16, 뒤에 흐린 글자 깔고 그 위에 투명 캔버스로 실제 따라 쓰기 가능)] + [빈 캔버스 2칸]. 기존 `_wBindCanvas`/`_wDrawGrid` 재사용(신규 음원·신규 캔버스 로직 없이 배선만 추가). "글쓰기 연습" 탭바에 "🔤 자모 획순 · Stroke Order"를 첫 탭으로 추가(`wr-stroke` 패널), `initWritingCanvas()`에 `initStrokeCanvases()` 호출 추가.
> — 좌표 1차 렌더링 후 스크린샷으로 시각 확인 → ㄷ·ㄹ·ㅁ·ㅂ·ㅅ·ㅌ·ㅍ에서 번호 원이 너무 가까워 겹쳐 보이는 문제 발견, 좌표 간격을 넓혀 재조정 후 재확인 완료.
> — ✅ **검증**: `node --check` 통과. Node vm으로 `STROKE_LETTERS.length===24`, 24자 전부 `STROKE_ORDER` 데이터 존재, 자음별 획수(ㅋ2·ㅌ3·ㅁ3·ㅈ2 등) 정확히 일치 확인. `buildStrokeOrder()` 출력에 `.stroke-row` 24개·`.stroke-cell` 144개(24×6)·`.stroke-canvas` 120개(24×5)·`.stroke-mark` 56개(전체 획수 합) 정확히 일치 확인. jsdom+node-canvas로 실제 DOM에 `buildWriting()` 렌더 → `initWritingCanvas()` 실행 → 120개 캔버스 전부 정상 바인딩, 마우스 드로잉 이벤트 시뮬레이션 및 전체 지우기(`clearStrokeCanvases()`) 정상 동작 확인. Playwright로 실제 스크린샷 렌더링해 24자 전부 번호 겹침 없이 육안 확인. CSS 중괄호 711/711, null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🐛 **2026-08-23 완료 (버그 수정 — "🌱 Start Here" 재클릭 시 이전에 보던 페이지로 튀던 문제)**
> — 선생님 발견: "Start Here에 들어와서 어떤 페이지를 방문하고 다른 레벨로 갔다가 다시 Start Here를 클릭하면 어떤 페이지가 보여야 되지?" → 코드 확인 결과 실제로 일관성 없는 버그였음. `setLevel()`이 Basics 진입 시 `loadBasics(curBasicsTopic)`을 호출하는데, `curBasicsTopic`은 "한글을 배워요"/"글쓰기 연습"/레퍼런스 항목처럼 `loadBasics()`로 여는 페이지를 클릭할 때만 갱신되고, 사이드바 맨 위의 **"🧭 학습 로드맵"**·**"🎯 배치 테스트"**는 별도 함수(`loadRoadmap()`/`loadPlacementTest()`)로 열려 `curBasicsTopic`을 전혀 안 건드림 — 그래서 로드맵을 보다가 다른 레벨 갔다 Start Here로 돌아오면 로드맵이 아니라 마지막으로 눌렀던 학습 topic(혹은 디폴트 'hangul')이 뜸.
> — 선생님 판단: 다른 레벨 버튼들은 전부 "항상 고정 디폴트(ep01)로 열림" 방식(2번)인데 Start Here만 "마지막 본 페이지로 돌아감"(1번) 방식이라 불일치 → **다른 레벨과 통일해서 Start Here도 항상 고정 디폴트로 열리게 하자**, 단 디폴트는 로드맵이 아니라 **"한글을 배워요"**로 지정(로드맵을 디폴트로 주면 안 된다고 명시).
> — `setLevel()`의 Basics 분기에서 `loadBasics(curBasicsTopic)` → `loadBasics('hangul')`로 변경. 이제 Basics 안에서 로드맵/배치테스트/다른 topic 중 뭘 보고 있었든, Start Here를 다시 누르면 항상 "한글을 배워요"로 열림(다른 레벨이 항상 ep01로 열리는 것과 동일한 패턴).
> — ✅ **검증**: `node --check` 통과. jsdom으로 실제 DOM(`ep-list`/`content-area`/버튼) 구성해 `setLevel()`을 실제로 실행하는 회귀 테스트 진행 — `curBasicsTopic`을 'writing'으로 미리 세팅(마지막으로 글쓰기 연습을 보고 있었던 상황 재현) 후 Start Here 버튼으로 `setLevel()` 호출 → 호출 후 `curBasicsTopic`이 'hangul'로 리셋됨을 확인, `content-area`에 "글쓰기 연습" 마커는 사라지고 "기본 자음"(한글을 배워요 콘텐츠) 마커가 나타남을 확인. null byte 0, `</html>` 1개, CSS 변경 없음.
> — ⏳ **git add/commit/push 대기 중**.

> 🔧 **2026-08-23 완료 ("이름과 소리" 탭 — Special Cases 내용 수정 + 음절 블록 그림 추가)**
> — 선생님 지적: "'이름이 살짝 다른 경우 · Special Cases'에 'ㄷ'은 아닌 것 같아. 'ㄷ'은 대표 소리 받침이라." → 맞는 지적. ㄷ은 `BATCHIM_GROUPS`에서 그 그룹 자체의 대표음(t)이라 이름↔소리가 어긋날 게 없음 (기역/리을 예시와 다를 바 없음). **ㄷ을 ㅅ(시옷)으로 교체** — 시옷은 이름이 '옷'(ot)이지만 받침일 땐 ㄷ그룹의 대표음 't'로 소리 나서, 기존 ㅈ(지읒)과 같은 종류의 진짜 특수 사례임.
> — `specials` 배열: ㄷ 항목 삭제, ㅅ 항목 신설(`initSyl:'시', initSnd:'s', finSyl:'옷', finSnd:'t'`, 한/영 노트 포함, 지읒 항목과 동일 구조).
> — `structure-note` 콜아웃을 ㅈ 단독 설명에서 **ㅅ·ㅈ 공통 설명**으로 일반화했다가, 선생님 재검토("이 설명이 이해를 도울까? 더 혼란스럽게 할까?") 결과 **다시 단순화**: "eus/euj/eut 직접 발음해보기" 식 음성학 실험과 "받침소리 규칙(7대표음)" 용어 재소환을 빼고, 받침 탭에서 이미 배운 내용을 "기억나세요?"로 복습하는 톤으로 전환. 실제 단어 예시(옷→[옫], 낮→[낟])로 대체. 링크 문구도 "7대표음 다시 보기"에서 "다시 보기"로 축약.
> — 선생님 요청: "이름과 소리에도 음절 블럭 그림이 들어가야지 초성, 종성 1, 4번호가 설명이 될 듯." → `buildPronouncePanel()` 리턴 문자열 맨 앞에 `sylBlockGuide('syllable')` 추가(음절 탭과 동일한 패턴), 인트로 문단에 "아래 카드의 (1), (4)는 위 그림의 위치 번호예요" 안내 추가.
> — 선생님 제안: "'이름이 살짝 다른 경우' → '이름이 살짝 다른 경우의 예시'라고 해야 이것 말고도 있다는 여운을 남기지 않을까?" → 맞는 지적. 지금 3개(이응·시옷·지읒)만 보여주는데 제목이 "경우"로 끝나면 전체 목록처럼 읽힐 수 있음. 섹션 제목을 "이름이 살짝 다른 경우의 예시 · Examples of Special Cases"로 변경해 대표 사례라는 뉘앙스로 조정.
> — ✅ **검증**: `node --check` 통과. Node vm으로 `buildPronouncePanel()` 재실행 → 리턴 문자열이 `sylBlockGuide` 마크업(`grid-template-columns:46px 46px`)으로 시작함을 확인, `시옷` 포함·`디귿`은 Special Cases가 아닌 자음 카드 섹션에만 정상적으로 남아있음 확인, 콜아웃에 음성학 전문용어(eus/euj) 없음 + "기억나세요" 복습 문구 + 실제 단어 예시 포함 확인, 섹션 제목 "...의 예시 · Examples of Special Cases" 반영 확인. CSS 변경 없음(모두 인라인 스타일 재사용), null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🧩 **2026-08-23 완료 (자음 탭 — 음절 블록 도형 앞에 "한글은 이렇게 생겼다" 기초 설명 추가)**
> — 선생님 지적: "한글을 처음 접하는 사람이 이 모형(sylBlockGuide)을 보면 뜬금없다 생각할 것 같아. 한글은 이렇게 생겼다고 알려줘야 되지 않을까? 이 정도도 모르고 시작할 수도 있으니." → 맞는 지적. 자음 탭이 학습 흐름상 첫 진입점인데, 도형이 아무 맥락 없이 바로 나와서 "왜 자음/모음이 2x2 격자에 배치돼 있지?"를 이해 못 할 수 있음.
> — `buildConsPanel()` 맨 앞, `sylBlockGuide('cons')` 호출 전에 기초 설명 문단 추가: "한글은 영어처럼 글자를 옆으로 쭉 늘어놓지 않아요. 자음과 모음이 모여 한 음절마다 네모난 블록을 이뤄요 (예: 한, 글). 아래 그림은 그 블록 안에서 각 글자가 어디에 오는지 보여줘요." + 영어 병기. 모음/받침/음절 탭은 자음 탭에서 이미 이 개념을 봤다고 가정하고 그대로 둠(중복 방지).
> — ✅ **검증**: `node --check` 통과. Node vm으로 `buildConsPanel()` 재실행 → 리턴 문자열이 새 기초 설명 문단으로 시작하고, 그 뒤에 기존 `sylBlockGuide` 도형 마크업이 그대로 이어짐을 확인. CSS 변경 없음(`basics-intro` 기존 클래스 재사용), null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🌐 **2026-08-23 완료 ("이름과 소리" 탭 — 설명 전체에 영어 번역 보완)**
> — 선생님이 새 탭 스크린샷을 보고 지적: "한글을 모르는 초급자니까 영어 설명 필수(영어권이라는 가정하에)" — 확인해보니 상단 인트로 문단들은 이미 한/영 병기였지만, 실제 설명 본문(기역/리을 예시 카드의 💡 노트, 이응/디귿/지읒 특수 사례 카드의 초성/받침 역할 설명)은 한국어만 있고 영어가 빠져 있었음.
> — `nameEx`(기역/리을) 각 카드의 💡 노트에 `<br><em>영어 설명</em>` 추가. `specials`(이응/디귿/지읒) 데이터에 `initNoteEn`/`finNoteEn` 필드 신설해 초성/받침 역할 설명 줄마다 영어 문장을 이어붙임(예: 이응의 초성 설명 "As the initial sound, it has no sound of its own — you only hear the vowel that follows."). 자음 카드 그리드의 🔊 첫소리/받침소리 버튼에도 `Initial`/`Batchim` 영어 라벨을 작게 추가(기존 `.wbtn-en` 방식과 같은 보조 텍스트 패턴).
> — `.pron-btn`을 가로 배치에서 세로 배치(라벨/영어/음절 3줄)로 CSS 변경, `.pron-btn-en` 신규 클래스 추가.
> — ✅ **검증**: `node --check` 통과, CSS 중괄호 702/702. Node vm으로 `buildPronouncePanel()` 재실행 → 예시 카드 노트에 `<em>` 영어 문장 포함, 이응/디귿 특수 카드 영어 노트 포함, 버튼에 Initial/Batchim 라벨 포함 각각 확인. null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🔊 **2026-08-23 완료 (한글을 배워요 — "🔊 이름과 소리" 신규 탭: 자음 이름→발음 원리 설명 + 첫소리/받침소리 듣기 카드)**
> — 배경: 선생님이 도란채 키즈(korean-app_v2.html) Level 1의 받침 탭 스크린샷을 보여주며 "자음마다 첫소리/받침소리를 들어볼 수 있게 만들었다"고 하셨고, 이어서 선생님이 직접 만드신 "How to Pronounce?" 참고 슬라이드(자음 이름의 앞음절=초성, 뒷음절=받침이라는 원리 + 이응/디귿/지읒 특수 사례 설명)를 보여주며 "음절 탭 다음에 새 탭을 만들어서 이 설명 + kids 자음카드(첫소리/받침소리)를 넣자"고 요청.
> — `buildHangul()`의 tabs 배열에 `['pronounce','🔊 이름과 소리']`를 음절과 연습 사이에 추가, `bp-hangul-pronounce` 패널 신설. 신규 `buildPronouncePanel()` 구성: ① 이름→발음 원리 예시 카드 2개(기역=초성 기(g)+받침 역(k), 리을=초성 리(r)+받침 을(l)), ② 특수 사례 카드 3개(이응·디귿·지읒 — 선생님 슬라이드 내용 그대로 반영, ㅈ받침이 왜 [t]로 나는지 설명 포함 + 받침 탭으로 바로 이동하는 링크), ③ 기본 자음 14개 전체를 카드로 나열, 각 카드에 **🔊 첫소리 / 🔊 받침소리** 버튼 두 개.
> — 새 음원 녹음 없이 **기존 두 음원 세트를 재사용**: 첫소리 버튼은 `HP_CONS_SOUND`(그·느·드... 연습모드용, 방금 ㅋ도 채움), 받침소리 버튼은 받침 탭의 7대표음 그룹 음원. 이를 위해 `buildBatchimPanel()` 안에 있던 지역 변수 `groups`를 전역 `BATCHIM_GROUPS`로 승격시키고, 자음→대표받침음 역참조 맵 `BATCHIM_SOUND_BY_CONS`를 신설해 두 탭이 같은 데이터를 공유하도록 리팩터(받침 탭 자체의 동작·출력은 그대로).
> — `core/nhs.css`에 `.pron-examples`/`.pron-example-card`/`.pron-specials`/`.pron-special-card`/`.pron-cards-grid`/`.pron-card`/`.pron-btn`(-init 틸·-final 코랄) 등 신규 클래스 추가. 색상은 첫소리=틸(기존 primary), 받침소리=코랄(기존 accent)로 앱 기존 팔레트 재사용.
> — ✅ **검증**: `node --check` 통과, CSS 중괄호 701/701. Node vm으로 `buildHangul()`에 신규 탭 마커·라벨 포함 확인, `buildPronouncePanel()` 실행해 카드 14개 전부 생성 확인, `BATCHIM_SOUND_BY_CONS` 매핑을 실제 JSON으로 덤프해 14개 자음 전부 대표음 그룹에 정상 연결됨을 확인(ㅋ→ㄱ그룹(윽), ㅅㅆㅈㅊㅌㅎ→ㄷ그룹(읃), ㅍ→ㅂ그룹(읍) 등). ㅋ 카드가 방금 추가한 `audio_36_크.mp3`를 정확히 참조하는지, ㅇ(첫소리 무음) 카드가 정상 렌더되는지 개별 확인. `buildBatchimPanel()`을 리팩터 후 재실행해 그룹 7개 그대로 출력되는 회귀 테스트 통과. null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🔊 **2026-08-23 완료 (자음 '소리' 음원 — 빠져있던 ㅋ 채워넣음)**
> — 배경: `HP_CONS_SOUND`(자음 연습 모드용, 이름이 아니라 실제 소리(그·느·드...)를 재생하는 음원 맵)에서 유일하게 `ㅋ`만 `audio` 필드가 없어 TTS 폴백으로 돌고 있었음 — 코드 주석에 이미 "ㅋ은 음원 없어 TTS 폴백"이라고 명시돼 있었던, 처음부터 알려진 갭(놓친 게 아니라 원본 TTS 녹음 배치 자체가 ㅋ을 건너뛰어서 생긴 빈자리). 실제 폴더(`data/elem/level1/TTS/consonant_combo/`)를 열어보니 0~35번 파일이 이름/소리 쌍으로 빈틈없이 이어져 있고 ㅋ만 이름(키읔)·소리(크) 둘 다 없는 게 확인됨.
> — 선생님이 ㅋ 소리 음원을 새로 만들어 오심 → `HP_CONS_SOUND`의 `'ㅋ'` 항목에 `audio:'data/elem/level1/TTS/consonant_combo/audio_36_크.mp3'` 추가(기존 0~35번과 번호 안 겹치게 36번 부여, 기존 파일들 재넘버링은 안 함 — 코드가 숫자로 순서를 계산하는 게 아니라 파일명을 그대로 참조하는 방식이라 번호 자체는 임의값이어도 무방). 이제 "ㅋ은 음원 없어 TTS 폴백" 주석도 제거.
> — ⚠️ **파일 배치 필요**: 선생님이 다운로드하신 음원 파일을 `halmoni-school/data/elem/level1/TTS/consonant_combo/audio_36_크.mp3` 경로에 직접 넣어주셔야 함 — Claude는 연결된 폴더 바깥(다운로드 폴더 등)엔 접근 불가.
> — ✅ **검증**: `node --check` 통과, `audio_36_크.mp3` 참조 정상 삽입 확인. null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중** (+ 위 음원 파일 배치 필요).

> 🧱 **2026-08-23 완료 (한글을 배워요 — 음절 탭에 "완성형" 번호 매긴 음절 블록 도형 추가 + 모음 캡션에 복합모음 케이스 보완)**
> — 알고 보니 선생님이 계속 보내주신 건 같은 PDF의 개정본들이었고, 최종적으로 PNG 4분할 비교 이미지로 "이 중 마지막(4번째) 완성형을 '음절' 탭에 넣어달라"고 요청. 4번째 카드는 앞의 3장(자음/모음/받침 강조용)과 달리 **네 칸이 전부 색칠되고 번호(1·2·3·4)로 발음 순서**를 보여주는 버전 — 초성(1) → 아래모음(2)/옆모음(3) → 받침(4, 있으면).
> — `sylBlockGuide(mode)`에 `'syllable'` 모드 추가: 네 칸을 모두 활성화하고, 기존 자음/모음/받침 모드의 한글 서브라벨(자음/모음/받침) 대신 **빨간 굵은 숫자(1~4)**를 표시하도록 `cell()` 헬퍼에 `numMode` 파라미터 추가(숫자용 별도 스타일). `buildSylPanel()` 최상단에 `sylBlockGuide('syllable')` 호출 추가 — 기존 SYL_VISUAL(천지인 설명+레고블록 예시)과 음절 조립기는 그대로 두고 그 위에 오리엔팅 도형만 얹음.
> — 겸사겸사 이전 항목에서 넣은 모음 탭 캡션도 선생님 피드백대로 보완: "오른쪽 또는 아래"뿐 아니라 **"둘 다"(ㅘㅝㅢ 등 복합모음이 옆+아래 두 자리를 동시에 씀)** 케이스를 추가.
> — ✅ **검증**: `node --check` 통과. Node vm으로 `buildSylPanel()`/`sylBlockGuide('syllable')` 직접 실행 → 네 칸 모두 하이라이트+번호(1/3/2/4, 그리드 위치별로 정확히 대조) 확인, 기존 SYL_VISUAL 콘텐츠("Lego block" 텍스트)가 그대로 남아있는지도 확인. 모음 캡션 "둘 다" 문구 포함 여부도 별도 확인. null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🧱 **2026-08-23 완료 (한글을 배워요 — 자음/모음/받침 탭에 "음절 블록 구조" 미니 도형 추가)**
> — 선생님이 참고 PDF("How to read Korean" 슬라이드, "Syllable Block Guide" 페이지)를 3차례에 걸쳐 공유(오타·크기 수정된 최종본까지) — 각 자음/모음/받침 탭에 "이 글자가 음절 블록 안에서 어디에 오는지" 보여주는 작은 2×2 위치 다이어그램을 넣어달라는 요청. PDF 최종본 기준으로 3가지 변형이 있었음: 카드1=자음(초성+받침 둘 다 강조), 카드2=모음(옆/아래 둘 다 강조), 카드3=받침만 강조 — 이 3분할을 그대로 반영.
> — 신규 함수 `sylBlockGuide(mode)` 추가(`mode:'cons'|'vowel'|'batchim'`) — 2행×2열+받침행 CSS 그리드로 초성(왼쪽 위)·모음(오른쪽=세로모음 자리 / 왼쪽아래=가로모음 자리)·받침(맨 아래, "(Optional)" 명시)을 그리고, 현재 탭과 관련된 칸만 색을 채우고 나머지는 회색 점선으로 흐리게 표시. **자음 탭은 초성 칸과 받침 칸을 둘 다 강조**(같은 자음이 초성으로도 받침으로도 온다는 점을 보여주기 위해, PDF 카드1과 동일한 의도), 모음 탭은 두 모음 칸을 모두 강조, 받침 탭은 받침 칸만 강조. 색상은 기존 음절(Syllables) 탭의 SYL_VISUAL 배색 계열과 맞춰 자음/받침=amber, 세로모음=green, 가로모음=blue로 통일. 각 다이어그램 옆에 한/영 캡션 문구 동봉.
> — `buildConsPanel()`/`buildVowPanel()`/`buildBatchimPanel()` 각각의 최상단에 `sylBlockGuide('cons'/'vowel'/'batchim')` 호출을 추가 — 기존 글자 그리드/설명은 그대로 두고 그 위에 오리엔팅 비주얼로 배치. 새 CSS는 추가하지 않고 전부 인라인 스타일 + 기존 CSS 변수(`--amber`,`--green-600`,`--blue`,`--gray-100` 등) 재사용.
> — ✅ **검증**: `node --check` 통과. Node vm으로 `buildConsPanel()`/`buildVowPanel()`/`buildBatchimPanel()` 직접 실행 → 세 패널 모두 다이어그램 마크업 포함 확인, `sylBlockGuide()` 세 모드 각각 실행해 하이라이트된 칸이 의도대로인지(자음=초성+받침 둘 다 amber, 모음=두 칸 모두 색칠, 받침=받침 칸만 amber) grid-area별로 직접 대조 확인. 전체 파일 sanity: null byte 0, `</html>` 1개, 파일 크기 464,790자.
> — ⏳ **git add/commit/push 대기 중**.

> 📏 **2026-08-23 완료 (빠른 참고 모달에 "띄어쓰기" 탭 추가 — QREF_TABS 6→7개)**
> — 배경: 이전 항목(사이드바 재편성)에서 "모달이 복잡해지니 essential/structure/spacing은 모달엔 안 넣기"로 정했었는데, 선생님이 "탭 하나 더 들어갈 여유가 있어 보인다"며 재검토 요청 → 실제로 모달 폭(최대 760px)과 기존 탭 6개 너비를 비교해보니 여유가 있는 게 맞아서 재논의.
> — 후보 3개(essential/structure/spacing) 중 처음엔 essential(필수 낱말 카드)을 추천했으나, 선생님이 두 가지 근거로 정정: ① essential은 이미 **색인(Index) 모달에도 "필수 낱말 카드"로 전부 들어가 있어 크로스 레퍼런스가 되어 있음**(제가 몰랐던 부분) → 빠른 참고에 또 넣으면 중복. ② **띄어쓰기는 매 에피소드 퀴즈에 나오는 내용**이라 학습 중 즉석에서 확인할 일이 실제로 많음 → 오히려 spacing이 "빠른 참고"의 원래 취지(공부하다 잠깐 열어보고 닫는 것)에 더 잘 맞음. 최종적으로 spacing으로 결정.
> — `QREF_TABS`에 `['spacing','📏 띄어쓰기']` 추가, `qrefRender()`의 분기에 `id==='spacing'?buildSpacing()` 추가 — `buildSpacing()`은 Start Here 사이드바용으로 이미 있던 함수를 그대로 재사용(콘텐츠 중복 작성 없음). structure/essential은 여전히 모달엔 안 넣고 사이드바 레퍼런스에만 유지.
> — ✅ **검증**: `node --check` 통과, Node vm으로 `qrefRender('spacing')` 직접 실행해 탭 7개 중 "띄어쓰기" 포함 확인 + 본문이 정상 렌더(에러 폴백 문구 아님) 확인. null byte 0, `</html>` 1개.
> — ⏳ **git add/commit/push 대기 중**.

> 🎨 **2026-08-23 완료 (사이드바 색 위계 정리 — 로드맵/배치테스트 테두리 제거, 한글기초 그룹 카드화 + active색 통일)**
> — 선생님 피드백 3단계로 진행: ① 로드맵·배치테스트 개별 박스 테두리 제거(배경은 유지하고 `border`를 `transparent`로만 바꿔 박스 크기·위치는 그대로). ② "한글 기초"를 낱개 박스 테두리 대신 **그룹 전체를 하나의 굵은 테두리 카드**로 묶음 — 신규 `.basics-hero-group`(2.5px 초록 테두리 카드) + `.basics-hero-group-hdr`(진한 초록 배경·흰 글씨 헤더 띠) + `.basics-hero-group-body`(그 안에 한글을 배워요·글쓰기 연습 두 항목, 개별 테두리는 없앰). 이제 로드맵(골드)·배치테스트(민트)·한글기초(초록, 큰 카드)로 3단 색 위계가 뚜렷해짐.
> — ③ 스크린샷 리뷰 중 발견: 카드 안 두 항목 중 기본 선택된("한글을 배워요") 것만 노란색으로 보이는 **버그**를 발견 — `.lt-item.active`(기존 로드맵류 공용 "선택됨=골드" 규칙, 클래스 2개라 specificity가 높음)가 `.lt-item-hangul`(클래스 1개)보다 우선 적용되면서 생긴 문제(이전엔 인라인 스타일이라 이 충돌 자체가 안 보였는데, 유지보수 위해 클래스로 옮기면서 드러남). `.lt-item-hangul.active{background:var(--sprout-lt);border-color:var(--sprout);}`를 추가해 specificity를 맞춰서(같은 2-클래스, 소스 순서상 나중에 정의됨) 선택 상태도 초록 계열 안에서 진하게만 바뀌도록 수정.
> — ✅ **검증**: `node --check` 통과, CSS 중괄호 677/677, null byte 0, `</html>` 1개. Node vm으로 `buildBasicsSidebar()` 재실행해 `.basics-hero-group`/`.basics-hero-group-hdr`/hero 항목 2개/레퍼런스 섹션 존재 확인. "내 이름 만들기" 전체 흐름(케이트 조립 등) 회귀 테스트도 재실행해 통과 확인.
> — ⏳ **git add/commit/push 대기 중**.
