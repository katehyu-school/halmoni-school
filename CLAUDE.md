...
> 🎮 **2026-09-06 완료 (한국어 방탈출 게임 파일럿 `dr-play.html` 신설 + nhs.html 에피소드 딥링크 신설)**
> — **어떻게 시작됐나**: 선생님이 "우리 앱 기반으로 증강현실 앱이 가능하냐"고 물으심 → 상상하신 건 AR이 아니라 **"우리 캐릭터와 대화하는 것"**이었음(메타버스 이미지). 정리 결과: ① VR/메타버스는 헤드셋 필요·효과의 핵심은 3D 세계가 아니라 "말을 해본다"는 것 ② 아이폰 사파리는 2026년 현재도 WebXR AR 미지원 ③ AI 대화는 지금 브라우저에서 되지만 **요금이 발생**(가장 싼 모델 기준 학습자 1명 매일 대화해도 월 30센트 수준, 구글 무료 등급 안에서 끝날 가능성 높음. 단 **Anthropic·Google 구독료로는 API가 안 됨**) → **선생님 선택: AI 없이 대사 트리(0원)로 먼저 만들고, 재밌으면 그때 AI를 얹는다.** 형식은 **방 탈출 게임**(선생님 아이디어 — 자물쇠가 "말"이 되어 못 넘어가게 하는 구조가 언어 학습과 잘 맞음).
> — **파일럿**: L1 ep06 「김밥 주세요」로 `dr-play.html`(단일 HTML, 38KB) 신설. **기존 자산 재사용** — 슬라이드 6장 배경 + 아줌마 TTS mp3 2개(`어서 오세요` / `잘 가요! 또 놀러 와요`), 없는 대사는 Web Speech 폴백. 3관문(들어가기 → 인사+주문 → 맛·계산·작별) + 살펴보기(메뉴판·보미·할머니 쪽지·천막) + 보미 힌트 + 엔딩 3종(🏆 실수0~1&힌트0 / 😅 그 사이 / 😳 실수 3+).
> — **핵심 설계**: 주문 조립기에서 ep06 문법 3개가 **전부 열쇠**가 됨 — 「인분」 안 쓰고 고유어(한 개)로 말하면 아줌마가 되묻고, 「하고/(이)랑/와과」 없이 두 개를 시키면 "떡볶이요? 김밥이요?"로 막힘. 조사는 **받침 판별로 자동 변형**(1인분**이랑** / 1인분**과**). 대사는 전부 파일 맨 위 `SCRIPT` 블록 한 곳에 모아 두어 **선생님이 말맛만 손보면 되게** 함(그 아래 ENGINE은 안 건드려도 됨).
> — ✅ **원작 오류 발견 → 같은 날 수정 완료**: ep06 슬라이드 속 메뉴판(`오뎅 1,000 · 떡볶이 3,500 · 순대 4,000 · 튀김 3,000 · 김밥 2,500`)대로면 할머니 심부름(떡볶이1+김밥2)은 **8,500원**인데 대사·TTS·퀴즈가 전부 「만오천 원」이었음(아주 초기 작업이라 그림 속 메뉴를 확인하지 않고 쓴 것). **선생님 판단: 8,500원이 맞다** → `data/nhs/L1/ep06.json` 7곳 수정(+13/-13, 값 교체만): 아줌마 대사 · 듣기퀴즈 q4의 `audio_text`·보기 4개·해설 · key_points 💰. **q4는 「만오천」이 정답이던 문제였으므로 보기를 재설계** — 정답을 `팔천오백`(index 2 유지, `answer` 안 건드림)으로 하고 **「만오천 원이에요」를 오답 보기로 남겨** 만(10,000) 단위 교육 포인트는 유지함. 슬라이드는 선생님이 새로 만들어 주심(`포장마차5.png` → 같은 이름 `포장마차5.webp`로 변환 교체, 960×540 유지, 88KB).
> — ✅ **ep06 가격 수정 완결(2026-09-07)**: 선생님이 슬라이드·음성을 새로 만들어 주셔서 전부 반영 완료. ① 슬라이드 — 첫 판은 말풍선이 「팔천 **오백**」으로 띄어져 있어 보고(한글 맞춤법은 수를 **만 단위로만** 띄어 씀 → 8,500은 「팔천오백」), 선생님이 붙여서 재작업 → `포장마차5.png` → 같은 이름 `포장마차5.webp`로 변환 교체(960×540, 88KB). ② 음성 — `8만오천+원이에요_slide5.mp3`를 「팔천오백 원이에요.」로 재녹음해 **같은 파일명에 덮어씀**(90,950B). ⚠️ **파일명은 옛 금액 그대로이니 다음 세션은 파일명만 보고 판단하지 말 것.** ③ `dr-play.html`의 `SCRIPT.g3_price`에 그 mp3를 연결 — 이제 게임에서도 아줌마 실제 목소리로 값을 말함.
> — 🧹 슬라이드 원본 PNG 2개는 `_to_delete/`로 옮겨 둠(git에 딸려 올라가지 않게) — 선생님이 그 폴더째 지우시면 됨.
> — 📐 **말풍선 여러 개 문제(범위만 재고 보류)**: 선생님 확인 — 슬라이드 생성 시 "말풍선 넣지 말라"는 지시를 빼먹으면 AI가 임의로 말풍선을 그려 넣었고, 초기 편들이 그렇게 됨. 실측 결과 **L1 ep02·03·04·05·06 다섯 편에 몰려 있고**(대사/슬라이드 비 1.5~2.0), 1장1말풍선으로 가면 **약 23장이 늘어남**. L2는 대체로 깨끗(1.08~1.20). **이번엔 범위 확장하지 않기로 함 — 나중에 할 때 이 숫자를 출발점으로.**
> — 🔗 **nhs.html에 에피소드 딥링크 신설(순수 추가 35줄)**: 게임 하단 "이 편 공부하러 가기" 링크가 ep01로 가버려서 확인해 보니 **nhs.html이 `?ep=`를 아예 안 받고 있었음**(`?view=start`/`?view=roadmap`만 처리, 나머지는 전부 `loadEp('ep01')`). `_openEpDeepLink()` 신설 — **`?ep=ep06` / `?ep=L2_ep03` (+ 선택 `&tab=vocab`)** 지원. `setLevel()`은 항상 그 레벨 ep01을 열어버려서 재사용하지 않고 레벨 표시(상단 탭·사이드바 헤더)만 직접 맞춘 뒤 `_doLoadEp()`를 부름 — **잠금 검사는 `_doLoadEp` 안에 이미 있으므로 중복하지 않음**(진입로마다 검사를 흩뿌리지 않는다는 2026-08-27 방침 유지). 앞으로 learn/·페이스북·QR 어디서든 특정 편으로 바로 보낼 수 있음.
> — 🐛 **내가 낸 버그와 교훈**: 그림이 안 나온다는 보고 → 파일·경로·유니코드(NFC) 전부 정상, 이미지도 실제로 로딩됨(960px)인데 화면엔 폴백이 떠 있었음. **원인은 `#sceneFallback{display:flex}`가 `hidden` 속성을 무력화**한 것(CSS `display` 지정이 UA의 `[hidden]{display:none}`를 이김). `#sceneFallback[hidden]{display:none}` 추가로 해결. **교훈: 요소가 보이는지 확인할 때 `el.hidden` 속성만 보지 말고 `getComputedStyle(el).display`로 볼 것** — 내 1차 검증이 속성만 봐서 통과시켜 버렸음.
> — **페이스북 공개 준비**: `og_play.jpg`(1200×630, 포장마차 슬라이드에서 생성) + OG/Twitter 메타 태그 → 링크 미리보기·카톡 공유 대응. `sitemap.xml`에 등록 + `tools/build_static_pages.py`의 루트 페이지 목록에도 추가(재생성해도 안 빠짐). **의도**: 문법 페이지는 아무도 안 퍼가지만 게임은 퍼감 → 색인이 안 되는 원인인 **외부 링크 부족**의 지렛대.
> — ✅ **검증**: `node --check` 통과, null byte 0, `</html>` 1개, `git diff --stat` = nhs.html +35/-0(순수 추가). 컨테이너 Playwright로 3관문·오답 6종·엔딩 전 구간 + 그림 정상/실패 두 상태를 **computed style로** 확인. **선생님 Chrome + Live Server에서 실사용 확인** — 딥링크 3종(`?ep=ep06` / `?ep=L2_ep03` / `?ep=ep06&tab=vocab`)과 무파라미터 회귀(ep01), 게임 그림 표시까지 눈으로 확인, 콘솔 에러 0건.
> — ⏳ **git add/commit/push 대기 중** — `nhs.html`·`dr-play.html`·`og_play.jpg`·`sitemap.xml`·`tools/build_static_pages.py`.
> — 🔜 **다음**: ① 선생님이 `SCRIPT` 대사 말맛 손보기(아줌마 되받는 말·보미 힌트·엔딩 문구) ② 재밌으면 **Kids 페이지에 링크**(번들 복호화→수정→재암호화 한 바퀴 필요하므로 다른 Kids 수정과 묶어서 한 번에) ③ 그 다음에 Real Life 탭의 새 타입(`type:'adventure'`)으로 정식 편입 검토 ④ 다른 편으로 확장.

> 🏠 **2026-09-07 완료 (대문 개편 — 밝은 색조로 전면 교체, `index.html` 적용)**
> — **결정**: 시안 2종을 만들어 비교한 뒤 선생님이 **시안 C(밝은 크림색)로 확정** ("나도 환한 배경이 더 좋은 것 같아"). 남색 시안 B는 채택 안 됨. 두 초안 파일과 따님 원본 시안은 `_to_delete/`로 옮겨 둠.
> — **바뀐 것**: 배경 남색→크림(#FDF6EC)·흰 카드·카드 위 색 띠(Kids 복숭아/도란채 민트)·**Kids 코랄 버튼, 도란채 틸 버튼** · 로고와 「도란채」를 **상단 줄**로 올림(큰 크레스트 머리글 숨김) · 상단 줄에 Free Lessons/Road Map/FAQ/Log in · 정체성 배지(`한국어 온라인 스쿨 · Korean Language School`) · 태그라인 **"Learn Korean through stories, not drills."**(따님 문구) · 숫자 칩 3개 · 카드 설명·칩 제거하고 **버튼 1개씩** · 아래 "실제 화면 3장"은 `display:none`으로 숨김(되살리려면 그 한 줄만 지우면 됨).
> — **동선**: 도란채 메인 `🌱 Start Learning Free`(부제 Open to everyone) → **`nhs.html?ep=ep01`**(Level 1) / 그 아래 `New to Korean? 한글 기초 · Hangeul Basics` → `?view=start` / Kids는 `🔑 Kids 입장하기`(부제 Invited families only) 하나만(카드 전체 클릭 제거) / 문법만 찾는 사람은 상단 Free Lessons → `learn/`.
> — **수치**: 첫 화면 클릭요소 0개 → 8개, 페이지 높이 1595px → 959px, **두 카드가 첫 화면 안에**(여유 36px). 나이 표기도 13+ → **14+**로 바로잡음(8월 확정 사항이 미반영돼 있었음).
> — 🐛 **작업 중 내가 낸 버그와 교훈**: 히어로의 큰 버튼을 파이썬 인덱스 슬라이싱으로 지울 때 **닫는 `</div>` 하나를 남겨서** `#landingBox`가 조기에 닫히고, 그 여파로 **로그인 후 화면(`#appBox`)이 `.wrap` 밖으로 빠져나가 왼쪽에 붙어 있었음**. 화면상 로그아웃 상태는 멀쩡해 보여서 못 잡을 뻔했고, `appBox.parentElement`를 찍어 보고서야 발견. **교훈: 마크업을 인덱스 슬라이싱으로 잘라낼 때는 반드시 `<div>`/`</div>` 개수 균형을 검사할 것**(지금은 세 파일 모두 90/90 확인). 그리고 **로그아웃 화면만 보지 말고 로그인 후 화면·모달까지 확인할 것.**
> — **로그인 후 화면·모달도 밝은 색조로 맞춤**: 환영 문구·수업 카드 그림자·로그인/가입/PIN 모달(흰 배경 + 크림색 입력칸 + 틸 버튼)까지 오버라이드. 수업 카드 자체 색(초등 민트/도란채 진초록)은 그대로 둠.
> — ⚠️ **확인 못 한 것**: 브라우저 창 크기 변경이 안 먹어서 **모바일 폭에서 눈으로 확인하지 못함**. 반응형 규칙(≤600px 카드 세로 쌓기, ≤640px 상단줄 세로 배치)은 코드로 확인했으나, 선생님이 휴대폰으로 한 번 봐주셔야 함.
> — 📝 **카드 설명은 다시 넣음(선생님 요청)**: 한 줄 설명이 아예 없으면 무엇을 배우는지 알 수 없어서 두 카드 모두 복원. 단 「first two levels free」류는 이미 칩·버튼 아래에 두 번 나와 겹치므로 빼고, **실제 장면**을 말하는 쪽으로 씀 — Kids: `Play-based lessons — from first sounds to full sentences.` / 도란채: `Follow one family through real scenes — markets, kitchens, campsites.` 좌우 줄 수가 달라지면 버튼 높이가 어긋나므로 `.track-desc`에 **두 줄 분 min-height**를 걸어 자리를 미리 확보함.
> — 📌 **남겨둔 것**: `learn/` 링크(117개 페이지로 가는 유일한 통로 — 빼면 색인이 더 나빠짐), 실제 화면 3장(숨김만).
> — ⏳ **git 커밋 대기** — `index.html`·`CLAUDE.md`. 되돌리려면 `git checkout index.html`(이전 남색 판이 커밋돼 있음).

> 🏠 **2026-09-07 진행중 (대문 개편 시안 2종 — 딸 자문 대기)**
> — 배경: 8월에 요청했던 **따님(전직 Meta UI) 자문이 도착**. `doranchae landing pg/Doranchae Landing.dc.html`(따님이 Claude Design으로 만든 시안). 요지 = ①버튼이 많아 혼잡 ②타겟(한글 모르는 사람)을 명확히 하고 영어로 읽기 쉽게.
> — **실측으로 확인한 것**: 현재 대문의 클릭 요소는 11개로 총량은 문제가 아니었고, 진짜 문제는 ⓐ **도란채 카드 하나에 버튼 5개가 세로로 쌓여 있던 것** ⓑ **첫 화면에 누를 수 있는 게 0개**였던 것(스크롤해야 첫 버튼). 따님이 "복잡하다"고 느낀 실체가 이것.
> — **만든 파일 2개(원본 `index.html`은 한 글자도 안 건드림)**:
>   · **`index_new.html` = 시안 B** — 우리 남색 정체성 유지 + 구조만 정리
>   · **`index_c.html` = 시안 C** — 따님의 밝은 크림색 색조에 우리 버튼·링크를 그대로 적용
>   둘 다 탭 제목에 `[시안 B]`·`[시안 C]` 표시. 구조는 동일하고 색조만 다름.
> — **두 시안의 공통 구조**: 로고·도란채 이름을 **상단 줄**로 올림(그 아래 큰 크레스트 머리글은 숨김) · 상단 줄에 Free Lessons/Road Map/FAQ/Log in · 정체성 배지(`한국어 온라인 스쿨 · Korean Language School`) · 태그라인 **"Learn Korean through stories, not drills."**(따님 문구 채택) · 숫자 칩 3개 · **카드마다 버튼 1개** · 아래 "실제 화면 3장"은 숨김. 페이지 높이 1595px → 946px, **두 카드가 첫 화면 안에** 들어옴.
> — **도란채 카드 동선 확정**: 메인 `🌱 Start Learning Free`(부제 "Open to everyone") → **Level 1 첫 편**(`?ep=ep01`) / 그 아래 `New to Korean? 한글 기초 · Hangeul Basics` → `?view=start`. Kids는 `🔑 Kids 입장하기`(부제 "Invited families only") 하나만(카드 전체 클릭 제거). 「그냥 둘러보기」는 메인 버튼과 같은 말이라 삭제.
> — ⚠️ **덤으로 발견**: 현재 `index.html`이 아직 **Ages 13 & up**으로 되어 있음(2026-08-14에 14+로 확정했는데 미반영). 두 시안에는 14+로 고쳐 뒀으나 **원본도 고쳐야 함**.
> — 📌 **일부러 남겨둔 것**: `learn/` 링크(117개 페이지로 가는 유일한 통로 — 빼면 색인이 더 나빠짐)와 실제 화면 3장(숨김 처리라 한 줄로 되살릴 수 있음).
> — ⏳ **다음**: 선생님이 **따님에게 두 시안을 보여주고** 의견을 받기로 함. 그 답이 오면 하나를 골라 `index.html`에 반영. 그 전까지 두 파일은 **로컬 초안**(git 미커밋, 사이트에 안 올라감).
