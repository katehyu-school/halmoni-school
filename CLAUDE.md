...
> 🧭 **2026-08-28 완료 (nhs.html — Start Here 로드맵 UX 개선: 상단 퀵 점프 + 아코디언 접기/펼치기)**
> — 선생님 요청: 로드맵 상단에 [0. 한글 기초] [1. 레벨 찾기] [2. 에피소드 학습법] [3. 매일 복습] [숨은 기능 & 루틴] 퀵 점프 버튼 + 'Step 2의 세부 8단계'·'모르면 그냥 지나치는 것들' 아코디언(기본 접힘, 요약만 노출) 적용 검토.
> — `loadRoadmap()`에 `_rmJump()`(퀵 점프 pill 버튼, 클릭 시 해당 섹션으로 smooth scroll)·`_rmToggleBtn()`/`_rmToggle()`(접기·펼치기 토글, 기본 `display:none`) 신설. Step 0~3 카드에 `id="rm-s0~3"` 부여(Step 4 마감테스트는 퀵 점프 대상에서 제외 — 요청하신 5개 버튼 그대로 반영), 📦 모르면 그냥 지나치는 것들 섹션에 `id="rm-hidden"` 부여(숨은 기능 & 루틴 버튼의 도착 지점 — 바로 아래 QR·학습자료·주간리듬까지 자연스럽게 이어짐).
> — Step 2의 8단계 상세 박스와 12개 기능표는 기본 접힘 + "자세히 보기 · Show details" 버튼으로 펼침(진행률 바·팁 문구는 계속 항상 노출).
> — 작업 방식: 대용량 파일 손상 이력(위 "작업 습관" 참고) 때문에 Edit 툴 대신 Python 앵커 치환(각 조각 고유성 count==1 확인) + `os.replace`로 원자적 저장. 한글 문단은 재입력 없이 원본 앵커만 잘라 감싸는 방식으로 처리해 오타 위험 원천 차단(단, 새로 추가한 버튼 라벨 문구는 코드포인트 검증 후 저장).
> — ✅ **검증**: null byte 0, `</html>` 1개, 추출한 `<script>` 블록 `node --check` 통과, `git diff --stat` = `nhs.html | 47 (+40/-7)`.
